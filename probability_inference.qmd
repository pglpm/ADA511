...
You can find an implementation of the algorithm above in the [R function `findP()`](https://github.com/pglpm/ADA511/blob/master/code/findP.R), which requires the `lpSolve` R package. This function takes as first argument the probability we want to find, and as remaining arguments the values of the given probabilities, or equalities among probabilities. We must use the following notation:
...
```
What does the result tells us about "combining evidence" to prove a hypothesis?
:::
