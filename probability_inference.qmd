...
$$
	the quantity is the same, but the meaning is very different. The probability can therefore be very different (if the engineer is the very person deciding the output, the probability is `100%`). The probability depends not only on a number, but on what it's being done with that -- measuring, setting, third-party reporting, and so on.
