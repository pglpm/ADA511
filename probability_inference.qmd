...
### Arriving at the target inference
...
we used the or-rule rewritten as follows:
...
The rules in fact represent, first of all, [*constraints of logical consistency*]{.blue}^[The technical term is [**coherence**]{.blue}.] among probabilities. For instance, if we have probabilities
...
- Translate the two constraints above verbally. Do they make intuitive sense? (For example, the first constraint says, very roughly speaking, that our belief in *two* things can't be stronger than the belief in either one of the two things alone.)
:::

\

In following the step-wise solution you may have been asking yourself "OK, why this step now, and not some other step?". This is a very intelligent question. You essentially noticed the *lack of an algorithm*. Compare this situation with the [solution of a basic decision problem](#sec-make-decision): there we had a clear sequence of steps to do: writing down some tables, do some element-wise multiplications and then some sums, and finally find the largest of a list of numbers.

Is there an algorithmic way of drawing inferences, that is, of calculating some target probabilities given some initial ones?

If our inference problem involves a *finite* number of sentences, then the answer is *yes*, but with some caveats.
...
This algorithm is not mathematically difficult, but it is somewhat involved. It boils down to:\ (1) writing the sentences in the initial and target probabilities in [*disjunctive normal form*](https://mathworld.wolfram.com/DisjunctiveNormalForm.html);\ (2) rewriting the initial and target probabilities as sum of probabilities of basic conjunctions;\ (3) solving two [*linear-fractional optimization*](https://reference.wolfram.com/language/ref/LinearFractionalOptimization.html) problems (for which there are algorithms available).

Obviously the algorithm becomes more expensive, the larger the number of sentences in the inference problem. Unfortunately the way the algorithm above works cannot be represented as a sequence of "logical" steps showing consecutive applications of the rules, as shown in the step-wise solution above. Later on in this course we shall focus on specific kinds of inferences for which other, less opaque inference algorithms are available.
...
- §12.2.3 in [*Artificial Intelligence*](references.html)
