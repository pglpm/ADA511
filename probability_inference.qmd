...
For this inference problem we can't find a `true` or `false` final value. The truth-inference rules (@eq-t-not)--(@eq-t-unity) therefore cannot help us here. In fact even the ["$\mathrm{T}(\dotso \| \dotso)$"]{.together} notation is unsuitable, because it only admits the values $1$ (`true`) and $0$ (`false`).
...
Then the probability of a sentence [$\yX$,]{.together} conditional on [$\yZ$,]{.together} is equal to a combination of probabilities conditional on $\yY_1,\yY_2,\dotsc$:
...
- In some inference problems we suddenly realize that we must distinguish between cases that depend on hypotheses, say $\yH_1$ and [$\yH_2$,]{.together} that were buried in the background information $\yI$. If the background information $\yI$ is explicitly reported in the notation, this is no problem: we can rewrite it as
$$ \yI = (\yH_1 \lor \yH_2) \land \yI'$$
and proceed, for example using the rule of extension of the conversation. If the background information was not explicitly written, this may lead to confusion and mistakes. For instance there may suddenly appear two instances of $\P(\yX)$ with *different* values, just because one of them is invisibly conditional on $\yI$, the other on [$\yI'$.]{.together}
