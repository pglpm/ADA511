...
> *Either you become a Bayesian or else you must live in a world with no general calculus of inference.*
\
[(Press,Teukolsky, Vetterling, Flannery: [*Numerical Recipes: The Art of Scientific Computing*](references.html))]{style="text-align:right;font-size:75%"} 
...
If we want to design an AI agent that can make decisions and act in uncertain situations, we need to equip it with *probabilities* and *utilities*. This is an inescapable necessity, as discussed in [§@sec-decision-theory]. It is intuitively clear what utilities are: they quantify how desirable different outcomes are to the agent. What do probabilities quantify?
...
{{< fa hand-point-right >}} This near-equality, 50%/50%, of beliefs *is different from the observed frequency*: the frequency right now is 100% `tails`. The frequency does affect Aisha's belief a little (she'd choose `tails` if offered an equal bet), but is *different* from her belief.
...
{{< fa hand-point-right >}} When frequencies are *available*, and come from *many* repetitions, then beliefs and frequencies tend to align.
...
{{< fa hand-point-right >}} Two agents may have different beliefs about the *same* situation; and yet both beliefs may be rational at the same time.

\

If you think that Aisha's and Aiden's behaviour and choices were rational, consider a situation in which two AI agents were in their place. It is apparent that we need to [**equip an AI agent with a sort of quantified, rational "degree of belief"**]{.blue} in order for it to take rational action in uncertain situations. This is also the result discussed in [§@sec-decision-theory]. **This degree of belief can have connections with an observed frequency, but is generally different from it.** The two notions must therefore be kept separate, and quantified separately. In fact, we may think of situations in which an agent may be *uncertain about a frequency* and needs to quantify its own belief about the frequency; we'll meet these situations in the [**Inference III**]{.green} part.
...
We shall take the notion of [*degree of belief*]{.blue} as intuitively understood, just as we did with the notion of truth. We shall use the terms [*probability*]{.blue}, [*degree of belief*]{.blue}, [*belief*]{.blue}, [*plausibility*]{.blue}, [*credibility*]{.blue}^[*credibility* literally means "believability" (from Latin *credo* = *I believe*).] as synonyms.
\
...
The algorithm described in the previous section is implemented in the R function [`inferP()`](https://pglpm.github.io/Pinference/reference/inferP.html); it requires the [**Pinference**](https://cran.r-project.org/package=Pinference) R package, which we installed in the [R introduction](Rintro1.html).
...
```{r}
library('Pinference') ## load the package
...
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious
Hailperin: [*Probability logic and combining evidence*](references.html).
:::
::::
