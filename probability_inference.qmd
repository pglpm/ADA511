...
The algorithm described in the previous section is implemented in the R function [`findP()`](https://github.com/pglpm/ADA511/blob/master/code/findP.R); it requires the [**lpSolve**](https://cran.r-project.org/package=lpSolve) R package, which we installed in the [R introduction](Rintro1.html).

The `findP()` function takes as first `target =` argument the probability we want to find, and as remaining arguments the values of the given probabilities, or equalities among probabilities. We must use the following notation:
...
source('findP.R') ## load the function
