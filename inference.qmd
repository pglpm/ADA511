...
In the assembly-line decision problem of [§ @sec-intro], the probability of early failure was very important in determining the optimal decision. If the probability had been $5\%$ instead of $10\%$, the optimal decision would have been different. Also, if the probability had been $100\%$ or [$0\%$,]{.together}, it would have meant that we knew *for sure* what was the successful decision.
...
- What are the rules for drawing inferences, that is, for calculating probabilities? And where are they drawn from?
...
:::{.column-margin}
![Films like [*The Matrix*](https://www.themoviedb.org/movie/603-the-matrix) ultimately draw on the fact that we must take some inferences for granted without further proof.](the_matrix.jpg){width=75%}
:::

In logic and mathematical logic, this corresponds to the fact that to prove some *theorem*, we must always start from some *axioms*. There are "inferences" -- *tautologies* -- that can be drawn without requiring others; but they are all trivial, such as "this component failed early, or it didn't". They are of little use in a real problem, although they have a deep theoretical importance.


## Basic elements of an inference

Let us introduce a first element of the mathematical notation for inferences.

We have seen that every inference has an "object" (what is to be assessed) and data and information on which it is based. We call  [**proposal**]{.blue}^[Johnson's (1924) terminology. Keynes (1921) uses "conclusion". Modern textbooks do not seem to use any specialized term.] the object of the inference, and [**conditional**]{.blue}^[Modern terminology. Other terms used: "evidence", "premise", "supposal".] what the inference is based upon. We separate them with a vertical bar^[Originally a [solidus](https://dictionary.cambridge.org/dictionary/english/solidus), introduced by Keynes (1921).]\ \ ["$\pmb{\|[\big]}$",]{.together}\ \ which can be pronounced *given* or *conditional on*:
$$
\textit{[proposal]}\ \pmb{\|[\Big]}\ 
\textit{[conditional]}
$$

To calculate the probability for such an inference, we start from those of other inferences. A basic inference process therefore can be schematized like this:

![](basic_inference2.png){width=100%}
