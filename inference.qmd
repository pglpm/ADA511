...
Every inference has an "object" -- what is to be assessed -- as well as data, information, or hypotheses on which it is based. We call  [**proposal**]{.blue} the object of the inference, and [**conditional**]{.blue} what the inference is based upon. We separate them with a vertical bar\ \ [" $\pmb{\pmb{\|[\big]}}$ ",]{.m} which can be pronounced "[**given**]{.blue}" or "[**conditional on**]{.blue}":
