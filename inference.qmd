...
In the assembly-line decision problem of [§@sec-intro], the probability of early failure was very important in determining the optimal action. If the probability had been $5\%$ instead of $10\%$, the optimal action would have been different. Also, if the probability had been $100\%$ or $0\%$, it would have meant that we knew *for sure* what was the successful action.
...
- What do we mean by "inference" and by "probability", more precisely? What's a probability? What important aspects about inferences and probabilities should we keep in mind?
...
@. Can any of the inferences above be done with full certainty (that is, to know which action is successful), based the data given explicitly and implicitly?
...
- Inferences are [typically based on some data and information]{.blue}, which may be explicitly expressed or only implicitly understood.
