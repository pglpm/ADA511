...
D.  The computer of a self-driving car needs to assess, from the current set of camera frames, whether a particular patch of colours in the frames is a person, so as to slow down the car and stop.
...
Every inference has an "object" -- what is to be assessed -- as well as data, information, hypotheses, or hypothetical scenarios on which it is based. We call  [**proposal**]{.blue} the object of the inference, and [**conditional**]{.blue} what the inference is based upon. We separate them with a vertical bar\ \ [" $\pmb{\pmb{\|[\big]}}$ ",]{.m} which can be pronounced "[**given**]{.blue}" or "[**conditional on**]{.blue}". Finally, we put parentheses around this and a "$\P$" in front:
...
![](probtree.svg){width=75% fig-align="center"}
