...
In the assembly-line decision problem of [§ @sec-intro], the probability of early failure (and that of late failure) was very important in determining the optimal decision. If the probability had been $5\%$ instead of $10\%$, the optimal decision would have been different. In that scenario the probability was given. In real decision problems, however, it almost always needs to be calculated, and its calculation can be the most time- and resource-demanding step in solving the problem.

We'll loosely refer to the problem of calculating probabilities as the "*inference* problem".
