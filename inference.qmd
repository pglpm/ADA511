...
3.  Looking at the present game position: ![](XsOs.png){width=5%} the X-player, which moves next, wonders whether placing the next [**X**]{style="color:#CA5556"} on the mid-right position leads to a win.
...
5. Given that $G=6.67 \cdot 10^{-11}\,\mathrm{m^3\,s^{-2}\,kg^{-1}}$, $M = 5.97 \cdot 10^{24}\,\mathrm{kg}$ (mass of the Earth) and $r = 6.37 \cdot 10^{6}\,\mathrm{m}$ (radius of the Earth), a rocket engineer needs to know [how much is $\sqrt{2\,G\,M/r\,}$](http://nasaphysics.cet.edu/escape-velocity.html).
