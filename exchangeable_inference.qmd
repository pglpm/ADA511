...
### How learning works under exchangeability
...
This fact becomes indeed clear if we check how the agent's beliefs about the frequency hypotheses change. In the cube-collection example of [§ @sec-freq-not-known], the agent has initial probabilities
...
Using the updated degree of belief above we also have another way to calculate the conditional probability $\P(C_{1} \mo \yy \| C_{2} \mo \yn \and \yul)$, using the derived rule of "extension of the conversation" in a different way:
...
\\[2ex]
\text{\grey\scriptsize(no learning if frequencies are known)}\quad
&=\sum_{\vf}
...
\\[2ex]
&=
{\green\frac{2}{3}} \cdot 66.667\%
+
{\green\frac{1}{2}} \cdot 33.333\%
\\[2ex]
&= \boldsymbol{61.111\%}
\end{aligned}
$$
