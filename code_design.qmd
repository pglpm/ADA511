...
: computes $\green\#\bz$ for all values $\bz$, as well as the multiplicative factor
...
The first problem is dealt with by rewriting the formulae in terms of logarithms, and renormalizing numerators and denominators of fractions. See for example the lines defining `auxalphas` in the [`buildagent()` function](https://github.com/pglpm/ADA511/blob/master/code/OPM-nominal/buildagent.R), and the line that redefines `counts` one last time in the [`infer()` function](https://github.com/pglpm/ADA511/blob/master/code/OPM-nominal/infer.R).

The second problem is dealt with by reorganizing the sums as multiples of identical summands; see the lines working with `freqscounts` in the [`buildagent()` function](https://github.com/pglpm/ADA511/blob/master/code/OPM-nominal/buildagent.R).
