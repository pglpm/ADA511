...
- `buildagent()` ([see code](https://github.com/pglpm/ADA511/blob/master/code/buildagent.R))
...
- `infer()` ([see code](https://github.com/pglpm/ADA511/blob/master/code/infer.R))
...
The first problem is dealt with by rewriting the formulae in terms of logarithms, and renormalizing numerators and denominators of fractions. See for example the lines defining `auxalphas` in the [`buildagent()`](https://github.com/pglpm/ADA511/blob/master/code/buildagent.R) function, and the line that redefines `counts` one last time in the [`infer()`](https://github.com/pglpm/ADA511/blob/master/code/infer.R) function.
