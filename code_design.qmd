...
- We shall call [*application*]{.blue} or [*instance*]{.blue} of the task a *single* inference about a new unit, for example a new incoming patient.
...
- It handles inferences and decisions about approximately infinite populations.

- It handles variates of *nominal* type ([§@sec-basic-types]).

- Its beliefs about the population are *exchangeable* ([ch. @sec-exchangeable-beliefs]).

- Its initial beliefs about the population are represented by a *Dirichlet-mixture distribution* ([ch. @sec-dirichlet-mix]):
...
Let's give an example of what we want our agent to be able to do. Suppose we have a population having three nominal variates $\bZ=(\bA \and \bB \and \bC)$. Abbreviate the set of training data as
...
We want an agent that draw inferences like these, as often as required:

- $P(\bB\mo\dotso\|\data \and \yD)$: inference about a predictand variate, without knowledge of any predictors.

- $P(\bA\mo\dotso \and \bC\mo\dotso\|\data \and \yD)$: same but for any two predictand variates.

- $P(\bA\mo\dotso \and \bB\mo\dotso \and \bC\mo\dotso\|\data \and \yD)$: same but for all three variates.

- $P(\bB\mo\dotso\|\bA\mo\dotso \and \data \and \yD)$: inference about any one predictand variate, given information about one predictor variate.

- $P(\bB\mo\dotso\| \bA\mo\dotso \and \bC\mo\dotso \and\data \and  \yD)$: same, but given information about any pair of predictors.

- $P(\bA\mo\dotso \and \bC\mo\dotso\|\bB\mo\dotso \and \data \and \yD)$: inference about any two predictand variates, given information about one predictor.

Note that *we are not fixing beforehand what are the predictands and what are the predictors*. Once the agent has learnt from the training data, we want to be able to change *on the fly, at each new application*, what the predictands are, and what the predictors are (if any).

Pause for a second and *ponder about the flexibility that we are requesting from our prototype agent!* Consider that virtually all present-day machine-learning algorithms only work one way: a machine-learning algorithm designed to guess the labels of images cannot guess an image given a label. Will we really manage to build an agent with the amazing versatility illustrated above?


## Computations needed and mathematical challenges in the code {#sec-code-computations}

The code implementing the OPM agent must essentially calculate
