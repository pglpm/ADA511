...
- It handles inferences and actions about approximately infinite populations, and its beliefs about the population are *exchangeable* ([ch. @sec-exchangeable-beliefs]).
...
- `decide()`
: asks an agent to choose between actions, specifying the list of possible actions, the predictands and their probabilities, and the utilities for the different actions and outcomes. We shall discuss this function more in detail in [ch. @sec-example-opm2].
...
- `plotFbelief()`
: plots, as a generalized ensemble plot, the possible full-population marginal frequency distributions for a single (not joint) predictand variate. If required it also plots the final probability distribution obtained with `infer()`.
