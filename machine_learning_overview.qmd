...
- unreasonably restrictive prior beliefs built into the algorithms.
...
This is in fact the law underlying formula @eq-definetti, which we used for our OPM agent. In that formula we recognize the sum over hypotheses as "$\sum_{\vf}$", with the difference that the hypotheses in that case are frequencies.

The sum over function hypotheses is computationally expensive, and the machine-learning agents above [*approximate*]{.red} it with something simpler: they [*keep only the largest term in the sum, rather than performing the sum*]{.red}:
...
You will see the appearance of this maximization, disguised as a minimization (of minus the logarithm of the probability), in the next machine-learning chapters.
...
This is why a perfect agent, like the OPM of the previous chapters, does not need a validation dataset: it does not approximate the required sum over hypotheses by an approximate maximization, and therefore does not need extra datasets to check how bad the approximation is.


### Unreasonably restrictive beliefs


In some situations the approximation above may not be too bad. But ofter



, the main sources are the following, in no specific order:

- [**Maximization, instead of summation, over possible frequencies**]{.red}.\ \ 

- [**Unreasonable built-in prior beliefs**]{.red}.\ \ The built-in prior beliefs can be unreasonable in two different ways:
    
    - The machine-learning algorithms above represent agents with the built-in belief that the functional relationship between time and temperature has as specific polynomial form. This belief may be unrealistic, although it leads to simpler computations than more realistic beliefs.
    
    - Within a particular complex functional relationship, the agent has too weak prior beliefs about which shapes could be more or less reasonable. The shape is therefore completely determined by the data. Note that we could have programmed a similar behaviour in the OPM agent, by choosing a very low $\amax$ parameter ([§@sec-intro-dirichlet-mix]). But even with such a programming, the OPM agent would have warned us that further learning data might considerably change its beliefs.
