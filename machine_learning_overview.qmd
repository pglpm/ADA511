...
We saw that the agent needs to have a built-in belief about frequencies, $\P(F\mo\vf \| \yI)$.\ \ Once it's given some learning data, it can in principle draw all sorts of inferences and take all sorts of actions. Concrete applications were given in chapters [-@sec-example-opm1], [-@sec-example-slm], [-@sec-example-opm2].


We shall now discuss about some present-day *approximate* procedures to draw inferences about data. They are approximate because they draw inferences or take action by doing approximate probability calculations and approximate maximizations of expected utilities, rather than exact ones. This approximating character makes these procedures very *fast*, but also *sub-optimal*. In any given problem it is therefore important to decide whether our priority is speed or optimality.
...
* If the action of a model on one data point affects the value of future data points, like for a self-driving car that may turn left and crash, or turn right and follow the road, we need a loss function that penalises certain actions and rewards others. This is called **reinforcement learning**.

<!-- We saw already in [chapter @sec-connection-3-ML] that this categorisation is not great, since there is always an input $x$ and an output $\hat{y}$, so the difference lies mostly in how we go about to find the optimal parameters. Still, from a practical perspective, it is good to know these three terms.  -->
...
The sum over hypotheses can be computationally expensive, and the machine-learning agents above [*approximate*]{.red} it with something simpler: they [*search for, and keep only the largest term in the sum, rather than performing the sum*]{.red}:
...
Search for "anchoring" and "availability heuristic" in Hastie & Dawes: [*Rational Choice in an Uncertain World*](references.html)
:::
::::

To make things worse, the factor $\P(F_{i} \| \yI)$, which should express prior information about the possible hypothesis $F_{i}$, is often poorly chosen or not chosen at all, and approximately the same for all hypotheses. So the maximum is effectively determined only by the factor $\P(\mathit{temp} \| \mathit{time} \and F_{i} \and \yI\bigr)$, which expresses how well hypothesis $F_{i}$ fits the existing data. This leads to overfitting.
...
Another way to mitigate the problem is by providing a non-constant prior-belief factor $\P(F_{i} \| \yI)$; this is what *regularization* procedures do. However, these procedures do not choose a value based on prior information; the choice is based on blind, trial-and-error attempts to reduce the influence of the data-fit term $\P(\mathit{temp} \| \mathit{time} \and F_{i} \and \yI\bigr)$. The result is again not optimal.

::::{.column-margin}
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious

MacKay: [*A practical Bayesian framework for backpropagation networks*](references.html)
:::
::::
