...
You can find a (less annotated) notebook associated with the present chapter: [`opm_adultincome_example.ipynb`](https://github.com/pglpm/ADA511/blob/master/code/OPM-nominal/opm_adultincome_example.ipynb).
...
Note that you can see the complete list of variates and their domains by simply calling `dimnames(opmall$counts)` (or any relevant agent-object name instead of `opmall`). Here is a snippet:
...
### Preliminary application: only predictands
...
### Preliminary application: specifying predictors
...
For instance, let's see three samples of how the full-population frequency distribution for $\mathit{sex}$ $\mathit{income}$ (jointly) could be:
...
In the example below, we generate 1000 frequency distributions for $(\mathit{occupation}, \mathit{sex})$ given $\mathit{income}\mo\cat{>50K}$, and then take the standard deviations of the samples as a rough measure of how much the probabilities we calculated a couple of cells above could change:
...
```{r}
## Calculate the joint probability for occupation and marital_status
probs <- infer(agent=opmall, predictand=c('occupation', 'marital_status'))
## Mutual information (in shannons) between same two variates
mutualinfo(probs=probs, A='occupation', B='marital_status')
```
