...
- calculates the probability of $\mathit{income}\mo\cat{<=50}$, via the function `infer()`
...
## index of 'income' variate
npredictand <- which(names(dimnames(opmall$counts)) == 'income')
##
stopwatch <- Sys.time() # record time
for(i in 1:ntest){
    probs <- infer(agent=opmall, predictor=testdata[i,-npredictand, with=F])
    testprobs[i] <- probs['<=50K']
    testhits[i] <- (decide(probs=probs) == testdata[i][['income']])
}
print(Sys.time() - stopwatch)
##
thist(testprobs, n=seq(0,1,length.out=10), plot=T,
      xlab='P(income = "<=50K")', ylab='frequency density in test set',
