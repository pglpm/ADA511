...
Consider again two populations briefly mentioned in [§ @sec-collections]

- Stock exchange
: The daily change in closing price of a stock during 1000 consecutive days. Each day the change can be positive or zero, $\green\cat{+}$; or negative, $\yellow\cat{-}$.
...
Each unit in these populations has some kind of tag which distinguishes it from all other units. In the stock-exchange population it is the day of the closing price; in the Mars-prospecting population it is the time the rock is entered into the database. Note that **this tag is *not* a population variate**. Units in any population always have some kind of tag: it could just be the row number of the database in which their data are stored.

Now suppose that the variate value for unit #735 in each of these populations is unknown to you (you're the agent) and you would like to infer it.
