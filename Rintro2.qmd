...
In these notes we use the custom function `pplot()`, which takes care of such conversions internally. This function is borrowed from the R package [**Prova**](https://pglpm.github.io/prova/), but we can load it from the file [`pplot.R`](https://github.com/pglpm/ADA511/blob/master/code/pplot.R), which you should have already downloaded:

```{r}
source('pplot.R')
```

Feel free to check the [documentation](https://pglpm.github.io/prova/reference/pplot.html) about this function.

Consider a quantity `animal` with six possible values, and  the distribution of probability over them, assigned to a variable `prob`:
...
With `pplot()` we can plot the probability distribution as follows:

```{r}
pplot(x = animal, y = prob1, type = 'b', ylim = c(0, NA))
```

Note how the animal names appear in the x-axis, in the order specified. The range specification `ylim = c(0, NA)` (not available for R base `plot()`) says that the lower range should be $0$, and the upper range should be the maximum available among the y-axis values.
\

Suppose we need to plot two different probability distributions (or two different sets of y-values more generally), stored in the variables `prob1` and `prob2`, for the same x-values, as in the last plot of [§@sec-discr-prob-distr]. We can do this by listing them together with the [`list()`](https://stat.ethz.ch/R-manual/R-patched/library/base/html/list.html) function, and then giving this list as the `y =` argument:

```{r}
prob2 <- c(0.2, 0.6, 0.05, 0.04, 0.01, 0.1)

pplot(x = animal, y = list(prob1, prob2),
    type = 'b', ylim = c(0, NA),
    ylab = 'two probability distributions')
```

We could also issue two separate `pplot()` commands, giving the argument `add = TRUE` to the second. But in this case we must make sure to give the correct y-range already in the first plot, and we must also explicitly use different colours `col =` and line types `lty =`. Check for instance what would happen for the case above:

```{r}
## Plot for prob1
pplot(x = animal, y = prob1,
    type = 'b', ylim = c(0, NA),
    ylab = 'two probability distributions')

## Plot for prob2
pplot(x = animal, y = prob2,
    type = 'b', col = 2, lty = 2,
    add = TRUE)
```
...
- Generate a large number of values, say hundreds, for $X$ from $-7$ to $10$. The number of values to generate depends on the resolution at which the final plot will be shown.
...
pplot(x = x, y = p, ylim = c(0, NA), xlab = 'x', ylab = 'p(x | I)')
```
...
## 1D ensemble plots in R {#sec-R-1Dscatter}

What if we want to represent a probability density as an ensemble plot, as was done in [§@sec-represent-dens]?
...
```{r}
samplesgauss <- rnorm(n = 100, mean = 5, sd = 3)
```
...
Let's use this function to add to the previous plot the sample points we just generated (we also redraw the plot); note that some sample points may land outside our plot's x-range:

```{r}
## Redraw the plot
pplot(x = x, y = p, ylim = c(0, NA), xlab = 'x', ylab = 'p(x | I)')

rug(samplesgauss, ticksize = 0.25, col = 2)
```

Our custom function `pplot()` can also draw 1D ensemble plots. In the present example we simply need to give it the sample points as the `x` argument and `NA` as the `y` argument:

```{r}
pplot(x = samplesgauss, y = NA)
```
...
```{r}
pplot(x = samplesgauss, y = NA, pch = '|')
```

Still another alternative is to generate many sample points and represent them as a cloud of dots, which extends also in the y-direction. This is done by using the arguments `pch = '.'` and `yjitter = TRUE`. Let's first generate 10 000 sample points and then plot them this way:

```{r}
## generate new sample points
samplesgauss <- rnorm(n = 10000, mean = 5, sd = 3)

pplot(x = samplesgauss, y = NA, pch = '.', yjitter = TRUE)
```

Finally, let's suppose we want two visualize and compare two different probability densities for $X$: the Gaussian distribution from before, and a [Cauchy distribution](https://mathworld.wolfram.com/CauchyDistribution.html). We can do this by generating sample points for both, and then plot them together with `pplot()` as discussed above.
...
Now we plot them together, displaying only the range from $X\mo -10$ to $X\mo 10$. We add labels for the two distributions as `y =` arguments. For both `x =` and `y =` argument we use `list()`. Let's also add some nicer x- and y-labels:

```{r}
pplot(
    x = list(samplesgauss, samplescauchy),
    y = list('Gaussian', 'Cauchy'),
    pch = '.', yjitter = TRUE,
