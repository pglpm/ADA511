...
In these notes we use the custom function `pplot()`, which takes care of such conversions internally. We can load it from the file [`pplot.R`](https://github.com/pglpm/ADA511/blob/master/code/pplot.R), which you should have already downloaded:

```{r}
source('pplot.R')
```

Consider a quantity `animal` that could be one of six possible animals, and  the distribution of probability over them, assigned to a variable `prob`. With `pplot()` we can plot the probability distribution as follows:
