...
Most inferences and decisions in engineering and data science involve things or properties of things that we can *measure*. We represent them by mathematical objects of different kinds. These objects have particular mathematical properties and can undergo particular operations.
...
:::{.callout-important}
## {{< fa exclamation-triangle >}} "Quantity" has very different meanings in different contexts
...
:::{.callout-important}
## {{< fa exclamation-triangle >}} "Quantity" vs "variate" vs "variable"
...
:::{.callout-important}
## {{< fa exclamation-triangle >}} Pay attention to symbol meaning and notation

- Keep in mind our discussion from [§@sec-sentence-notation]: we must make clear what "$\mo$" means. It could mean "observed", "set", "reported", and so on.

- Note the subtle difference between [$X$]{.black}, in italics, and [$\se{X}$]{.black}, in sans-serif. The first denotes a *quantity*, the second denotes a *sentence*. Usually we don't have to worry too much about these symbol differences, because the meaning of the symbol is clear from the context. But just in case, you know the convention.
:::
...
## Importance of metadata for inference and action {#sec-metadata}
...
An AI agent or machine-learning algorithm will therefore make better guesses and take better actions if it is given full metadata, besides "training" data. For this reason metadata are extremely important for inference and decision-making, and an optimal agent should make use of metadata.
