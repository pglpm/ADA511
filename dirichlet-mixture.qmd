...
so the sum $\sum_k$ runs over $2\cdot 17 +1= 35$ terms.
...
- Try doing the calculation above on a computer, with $\amin=-17, \amax=17$. What happens?
...
- Try doing the calculation above on a computer, using the values\ \ $\amin=-17, \amax=17$.

    If you use the formulae above as they're given, you'll probably get just `NaN`s. The formulae above must be rewritten in a different way in order not to generate overflow. The result would be $\boldsymbol{52.26\%}$.
...
The agent has learned an association between $\ambu$ and $\urge$ from the first patient. Note that if we had used the parameters $\amin=-17, \amax=17$, the result would have been more conservative: $\boldsymbol{58.11\%}$.
...
- It can be "eager" to learn from previous examples, that is, its probabilities may vary appreciably even with only few observations. The "eagerness" is determined by the parameters $\amin, \amax$. For a general-purpose agent, the values $\amin=-17, \amax=17$ are more reasonable.
