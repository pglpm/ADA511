...
Suppose we have a simple or joint nominal variate $Z$ which can take on $M$ different possible values; these can be joint values, as in the examples of [§@sec-categ-probtheory]. As usual $\vf$ denotes a specific frequency distribution for the variate values. For a given value $\bz$, we denote by $f(\bz)$ the relative frequency with which that value occurs in the full population.
...
Besides some multiplicative constants, the probability of a particular frequency distribution is simply proportional to the product of all its individual frequencies, raised to some powers. The product "$\prod_{\bz}$" is over all $M$ possible values of $Z$. The sum "$\sum_{\ya}$" is over an *integer* (positive or negative) index $\ya$ that runs between the minimum value $\amin$ and the maximum value $\amax$. In the applications of the next chapters these minimum and maximum are chosen as follows:
...
The variate $\yR$ can take on two values $\set{\yy,\yn}$, so $M=2$ in this case. The frequency distribution consists of two frequencies:
...
We can visualize this belief distribution (with $\amin=0, \amax=2$) with a generalized ensemble plot ([§@sec-repr-general-distr]) of 100 frequency distributions; each distribution is represented by a line (recall [§@sec-discr-prob-distr]):
...
where $\#\bz$ is the multiplicity with which the specific value $\bz$ occurs in the sequence $z_1,\dotsc,z_{L}$.
