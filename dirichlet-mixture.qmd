...
A mathematical advantage of the Dirichlet-mixture belief distribution is that some probability formulae that contain it can be computed exactly. The most important formula is de Finetti's representation theorem ([§@sec-freq-not-known]), where the sum "$\sum_{\vf}$" (really an integral "$\int\,\di\vf$") can be calculated analytically in the case of the Dirichlet-mixture:
