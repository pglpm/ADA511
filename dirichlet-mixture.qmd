...
The parameters $\amin, \amax$ encode, approximately speaking, the agent's prior belief about the size of a "representative sample" of the full population ([§ @sec-samples]). More precisely, $2^\amin$ and $2^\amax$ represent a lower and an upper bound of what the representative sample could be. Values\ \ $\amin=0$,\ \ $\amax=20$\ \ therefore represent a belief that the representative-sample size could be anywhere between 1 unit and approximately 1 million units. The belief is spread out uniformly across the orders of magnitude in between.

If $2^\amin$ is larger than the amount of training data, the agent will consider these data insufficient, and tend to give uniform probabilities to its inferences; for instance a 50%/50% probability to a binary variate.

If the order of magnitude of a representative sample is known, for example from previous studies on similar populations, the parameters $\amin,\amax$ can be set to that order of magnitude (in base 2), minus or plus some magnitude range.

If such an order of magnitude is not known, then it does *not* make sense to "estimate" it from training data with other methods, because an agent with a Dirichlet-mixture distribution will already do that internally (and in an optimal way), provided an ample range is given with $\amin, \amax$.
...
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious
[*Prior probabilities*](https://hvl.instructure.com/courses/25074/modules/items/723829
)
:::
::::
...
- The joint variate $Z$ only consists of few simple variates.
...
- [*Monkeys, kangaroos, and $N$*](https://hvl.instructure.com/courses/25074/modules/items/724821): is an insightful discussion of how to investigate and represent prior beliefs.
:::
::::
