...
A mathematical advantage of the Dirichlet-mixture belief distribution is that it allows us to compute this belief distribution in an exact way. Start from de Finetti's representation theorem ([§@sec-freq-not-known]) and use the Dirichlet-mixture [@eq-dirichlet-f] for $\P(F\mo \vf \| \yI)$. The integral "$\int\,\di\vf$" can be solved explicitly and we find the following result:
...
Let's see how to concretely use the formula above in an example from the Mars-prospecting scenario. Take the sequence^[Remember that the agent has exchangeable beliefs, so the units' IDs don't matter ([§@sec-exchaneable-distr])!]
...
We also have $M=2$ (two possible distinct values), and we still take $\amin=0, \amax=2$. The formula above then becomes
...
[{{< fa 3 >}} --]{.green}\ \ \ The probability for the hypothesis of interest is finally given by the fraction
...
[{{< fa 2 >}}{{< fa a >}} --]{.green}\ \ In the first joint probability above the value pair $(U\mo\urge \and T\mo\ambu)$ appears once, and the remaining five pairs appear zero times:
...
[{{< fa 3 >}} --]{.green}\ \ \ The probability that this incoming patient is urgent is then
...
[{{< fa 1 >}} --]{.green}\ \ \ The hypotheses are again $U_3\mo\urge$ and $U_3\mo\nonu$, and we need the joint probabilities
...
[{{< fa 3 >}} --]{.green}\ \ \ Finally, the probability that the third incoming patient is urgent is
...
The agent has learned an association between $\ambu$ and $\urge$ from the first patient. Note that if we had used parameters $\amin=0, \amax=20$, the result would have been more conservative: $\boldsymbol{54.90\%}$.
