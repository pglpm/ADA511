...
- In absence of previous data, it gives uniform distributions to any variate.

- It is "eager" to learn from previous examples; in other words, its probabilities vary appreciably with even just a few observations.

These characteristics can be appropriate to some inference tasks, but not to others. Note again that this is a matter of *background information* about the task one wants to solve.

This kind of agent is reasonable in situations where:
...
- A large number of previous data is available, "large" relative to the domain size $M$ of the total variate $Z$.
