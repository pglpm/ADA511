...
so the sum "$\sum_k$" runs over 21 terms. In most applications it does not matter if we take a lower $\amin$ or a higher $\amax$.
...
To consider the general case, suppose we have a population with variates $Y$, which can take on $M_Y$ possible values, and $X$, which can take on $M_X$ possible values, for a total of $M = M_Y \cdot M_X$ possible joint values. Also suppose that the agent has learned the values of $N$ units, which we collectively call $\data$:

$\data \defd
( Y_{N}\mo y_{N} \and  X_{N}\mo x_{N} \,
\and\dotsb \and \,
Y_{1}\mo y_{1} \and  X_{1}\mo x_{1})$

Use also the following notation:

- $\#(y, x)$ is the number of times (the absolute frequency) with which the specific *pair* of values $(y, x)$ appear in the data.
- $\#y$ is the number of times with which the specific value $y$ of variate $Y$ appears in the data.
- $\#x$: ditto, but for $X$.

For example, if $Y$ has domain $\set{\urge, \nonu}$ and $X$ has domain $\set{\ambu, \othe}$, and our data are

::::{.column-page-inset-right}
$\data = (
Y_{3}\mo\urge \and X_{3}\mo \ambu \and
Y_{2}\mo\urge \and X_{2}\mo \othe \and
Y_{1}\mo\nonu \and X_{1}\mo \ambu
)$
::::

then we have $M_X = 2$, $M_Y = 2$, $M = 4$, $N = 3$, and
...
Using this notation, it is possible to prove that following conditional-probability formulae for the Dirichlet-mixture agent:
...
\frac{
\frac{
\prod_{x,y} \bigl(\frac{2^{\ya}}{M} + \#(x,y) - 1\bigr)!
}{
\bigl(2^{\ya} + N -1 \bigr)!
}
\cdot
...
\sum_{l = \amin}^{\amax}
\frac{
\prod_{x,y} \bigl(\frac{2^{l}}{M} + \#(x,y) - 1\bigr)!
}{
\bigl(2^{l} + N -1 \bigr)!
}
\cdot
