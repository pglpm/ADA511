...
They do. Indeed they are regularly used in Communication Theory and Information Theory, owing to the properties above. They even have [international standards](https://www.iso.org/obp/ui/en/#!iso:std:63598:en) on their definition and measurement units. Here are the two most useful for inference purposes
...
- The value (in shannons) of the conditional entropy is the number of binary digits that we lack for correctly identifying the value of $Y$, if the possible values were listed as integers in binary format. Alternatively it can be interpreted as the minimal number of divisions necessary to find the correct value of $Y$ in a binary search.
...
The conditional entropy for $Y$ given $X$ can be misleading in one respect. Suppose that for any value of $X$ there's a 100% probability that $Y$ has one and the same value, say $Y\mo 1$. The conditional entropy is then zero. In this case it is true that $Y$ is formally a function of $X$. But it is also true that we could perfectly predict $Y$ without any knowledge of $X$. The observed value of $X$ didn't really help us in forecasting $Y$; in other words, $X$ is not relevant for inference about $Y$.^[There's no contradiction with the second "remarkable property" previously discussed: in this case the maximal value that the conditional entropy can take is zero.]

This misleading aspect can be corrected by subtracting the conditional entropy for $Y$ given $X$, from the maximum value it could take if $X$ were unavailable. The result gives a measure of how much $X$ effectively "helped" in inferring $Y$.
...
and, as defined above, is also measured in *shannons*.

For instance, in the case of [table @tbl-distr-entropy], the mutual information has the following values in the three scenarios:

1. $\HH(Y : X\| \yI_1) = 1.61\,\mathrm{Sh}$ , almost zero; indeed $Y$ can almost be considered a function of $X$ in this case.

2. $\HH(Y : X\| \yI_2) = 0.81\,\mathrm{Sh}$ , almost 1; indeed in this case we are approximately uncertain between two values of $Y$.

3. $\HH(Y : X\| \yI_3) = 1.5\,\mathrm{Sh}$ ; indeed this case is intermediate between the previous two.
