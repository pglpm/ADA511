...
- The value (in shannons) of the Shannon entropy can be interpreted as the number of binary digits that we lack for correctly identifying the value of $Y$, if the possible values were listed as integers in binary format. Alternatively, a Shannon entropy equal to $h\,\mathrm{Sh}$ correspond to being fully uncertain among $2^h$ possible alternatives.
