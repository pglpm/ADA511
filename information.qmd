...
- The value (in shannons) of the Shannon entropy can be interpreted as the number of binary digits that we lack for correctly identifying the value of $Y$, if the possible values were listed as integers in binary format. Alternatively it can be interpreted as the maximum number of iterations necessary to find the correct value of $Y$ in a binary search. But many other interpretations exist.
...
- The value (in shannons) of the conditional entropy is the number of binary digits that we lack, on average, after $X$ is known, for correctly identifying the value of $Y$ if the possible values were listed as integers in binary format. Alternatively it can be interpreted as the maximal number of binary-search iterations necessary to find the correct value of $Y$, once $X$ is known.
...
- The value (in shannons) of the mutual information is the number of binary digits provided, on average, by knowledge of $X$, for correctly identifying the value of $Y$ if the possible values were listed as integers in binary format. Alternatively, it is the average number of iterations that knowledge of $X$ saves us from doing, in a binary search for finding the value of $Y$.
...
1. $\HH(Y : X\| \yI_1) = 1.61\,\mathrm{Sh}$ , almost equal to the maximal value achievable in this scenario ($1.62\,\mathrm{Sh}$); indeed $Y$ can almost be considered a function of $X$ in this case.

2. $\HH(Y : X\| \yI_2) = 0.81\,\mathrm{Sh}$ ; this means that knowledge of $X$ gives us slightly less than a binary digit of the value of $Y$.

3. $\HH(Y : X\| \yI_3) = 1.5\,\mathrm{Sh}$ ; knowledge of $X$ gives us one and a half binary digits of the value of $Y$.
