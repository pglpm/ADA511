...
- The value (in shannons) of the conditional entropy is the number of binary digits that we lack for correctly identifying the value of $Y$, if the possible values were listed as integers in binary format. Alternatively it can be interpreted as the minimal number of divisions necessary to find the correct value in a binary search.
