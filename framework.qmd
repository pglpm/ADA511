...
The basic idea is that the agent will [**infer**]{.text-warning} the outcomes given the assumptions, with some degree of [**uncertainty**]{.text-warning}. From these inferences it will determine the [**optimal**]{.text-warning} decision.

:::{.callout-tip}
## Example
> A particular kind of electronic component is produced in an assembly line. At the end of the line there is an automated tester that makes some tests on every new component. The tests should give an indication whether the component will fail within less than a year or only after a year, with some probabilities. For a particular new component, for instance, there turns out to be a 10% probability of failure within a year.
>
> Depending on the test results, the automated tester approves the component for sale, or discards it. A sold component gives a net gain of 1\$, if it works for at least a year; but if it fails within a year, it leads to a net *loss* of 10\$, owing to damage and warranty refunds. A discarded component doesn't lead to any gain or loss.
\

A decision problem is involved in this process, for every new component that arrives at the tester. The elements of the problem can be identified as follows:

- *Agent*: the automated tester (and indirectly the engineer who designed and programmed it).
- *Assumptions*: the values obtained with the various tests.
...
- *Probabilities*: 10% probability of failure within a year, if the component is sold, and 90% of functioning correctly. If the component is discarded, nothing further happens.
- *Decisions*: approve the component, or discard it.
- *Utilities*: 1\$ for approval without failure, -10\$ for approval with failure, 0\$ for disposal.
:::
