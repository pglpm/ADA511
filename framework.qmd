...
Let us emphasize, however, that we are not giving up on "success", or trading it for "optimality"! Indeed we'll find that **the method automatically leads to the *successful* decision** in problems where uncertainty is not present or is irrelevant. It's a win-win. It's therefore important to keep in mind this point:

::::: {.column-page-inset-right style="color:#4477AA;"}
:::: {.callout-note appearance="default" icon=false style="color:#4477AA;"}
##
[Aiming to find the solutions that are *successful* makes us *fail* to find those that are optimal when the successful ones cannot be determined.]{style="font-size:125%"}

[Aiming to find the solutions that are *optimal* makes us automatically find those that are *successful* when they can determined.]{style="font-size:125%"}
::::
:::::
