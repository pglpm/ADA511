...
In each of the 10 imaginary repetitions where the component fails early, the manufacturer loses $11\$$. That's a total loss of $10 \cdot {\red 11\$} = {\red 110\$}$. In each of the 90 imaginary repetitions in which the component doesn't fail early, the manufacturer gains $1\$$. That's a total gain of $90 \cdot {\green 1\$} = {\green 90\$}$. So over all 100 imaginary repetitions the manufacturer gains

$$
10\cdot ({\red -11\$}) + 90\cdot {\green 1\$} = {\red -20\$}
$$
...
Obviously the best, or "least worst", action to take is to **discard** the component.
...
@. Consider the special case with failure probability `0%` and failure cost `10$`. That probability means that no new component will ever fail. It's clear what's the optimal action in this limit case, without any calculations or imaginary repetitions. Yet, **confirm mathematically** that we arrive at this obvious conclusions if we perform a mathematical analysis like before.
...
- Is it enough if we simply believe that the component is less likely to fail than not? In other words, is it enough if the probability of failure is *less than 50% without knowing its precise value*?
    ...
    Obviously not. We found that if the failure probability is 10% then it's best to discard. But we also found that if it's 5% then it's best to accept. In either case the probability of failure was less than 50%, but the decision & action was different.

	On top of that, we also found that the probability value determines the average amount of loss when the non-optimal action is taken. Therefore:

	[**{{< fa hand-point-right >}} Knowledge of *precise* probabilities is absolutely necessary for making the best decision and taking the best action.**]{.blue}
\
\
...
- Is it enough if we simply know that failure leads to a loss, and non-failure leads to a gain, without knowing the precise amounts of loss and gain?
	...
	[**{{< fa hand-point-right >}} Knowledge of the precise gains and losses is absolutely necessary for making the best decision and taking the best action.**]{.blue}
\
\

- Is this kind of decision-making situation only relevant to assembly lines and sales?

    By all means not. We examined a clinical problem that's exactly analogous: there's uncertainty and probability, there are gains and losses (of lifetime rather than money), and the best action depends on both: probabilities *and* costs.
...
## Our focus: decision-making, inference, data science, AI {#sec-our-focus}
...
- {{< fa cube >}} There is a decision or choice to be made, or action to be taken (and "not deciding" is not an option, or it's just another kind of choice).

- {{< fa cube >}} Taking a particular action will lead to some consequences. Some consequences are desirable, others are undesirable.
...
This is what we call a problem of [**decision-making under uncertainty**]{.blue} or **under risk**^[We avoid the word "risk" because it has several different technical meanings in the literature, some even incompatible with one another.]; or simply a "decision problem" for short.

This problem-pattern appears literally everywhere. Think about all different situations in which you had to make a decision today. Do they show this pattern?
...
## The broad meaning of *decision*; universality of decision-making in Data Science and AI {#sec-meaning-decision}
...
In fact the term [*course of action*]{.blue} or simply *action* is also commonly used in decision theory, instead of *decision*.

[**It is this more general sense of "decision" and "decision-making" that we study in this course.**]{.blue} In this more general sense, decisions permeate every second of any human being and of any agent. **Decision-making of all kinds occurs continuously and determines the behaviour of any agent equipped with Artificial Intelligence.**

Thus, by studying decisions and decision-making, we're studying how AI agents should behave and act.
...
By definition, in a decision problem under *uncertainty* there is generally no method to determine the action that *surely* leads to the desired consequence. If such a method existed, the problem would not have any uncertainty! Therefore, if there is a method to deal with decision problems, its goal cannot be the determination of the *successful* action. <!-- This also means that, a priori, we cannot blame an engineer for making an unsuccessful decision in a situation of uncertainty. --> Then what should be the goal of such a method?
...
- ["tails-bet"]{.lightblue}: If the coin lands [tails]{.green}, the person *wins* a [small amount]{.green} of money. If it lands [heads]{.red}, they lose the same [*small* amount]{.red} of money.
...
Now consider this: *you know* the "truth", you know what the successful decision would be: heads-bet. It turns out that the [Henry]{.smallcaps} algorithm made the choice corresponding to the truth. The [Tina]{.smallcaps} algorithm didn't. Would you then evaluate the [Henry]{.smallcaps} algorithm to be better than the [Tina]{.smallcaps} algorithm?

For exactly the same reasons already discussed, the [Tina]{.smallcaps} algorithm is the better one; it made the optimal decision. Yet it didn't choose the "truth". You realize that *comparing algorithms is not as simple as checking which one yields the truth*.
\

We have then arrived at two conclusions:

- {{< fa hand-point-right >}} "Success" or "correspondence to truth" is generally **not** a good criterion to judge a decision under uncertainty or to evaluate an algorithm that makes such decisions. Moreover, in real applications the truth is *not* known -- that's the whole problem! -- so how can we use a criterion based on truth?

<!-- Success can be the pure outcome of luck, not of smarts. -->
...
[Aiming to find the solution that is *optimal* makes us automatically find the solution that is *successful*, when this can be determined.]{.green style="font-size:120%"}
::::
:::::

We shall later witness this fact with our own eyes. We will also take it up in the discussion ([chapter @sec-eval-decision]) of some misleading techniques to evaluate machine-learning algorithms.
...
- {{< fa check >}} It takes into consideration decisions and actions, consequences, costs and gains.
...
- If we find an optimal action in regards to some problem, it may still happen that this action leads to new, subsequent decision problems. For example, in the assembly-line scenario the action `discard` could be carried out by burning, recycling, and so on. And each of these actions could have uncertain results and costs or gains. We thus face a decision after a decision, or a decision within a decision. In general, a decision problem may involve several decision sub-problems, in turn involving decision sub-sub-problems, and so on.

- In data science, a common engineering goal is to design and build an *automated* AI-based device capable of taking an optimal action, at least in specific kinds of uncertain situations. Think for instance of an aeronautic engineer designing an autopilot system; or a software company designing an image classifier.

Well, Decision Theory turns out to meet these two demands too, thanks to the following features:

:::{.green}

- {{< fa check >}} It allows for recursive, sequential, and modular application.
