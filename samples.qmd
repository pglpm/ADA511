...
[*Representative Sampling, I*](https://hvl.instructure.com/courses/25074/modules/items/715303)
:::
...
For some populations, the mean and standard deviation calculated in a sample can be wildly different from those of the full population -- even when the sample comprises half of the full population! Here is a demonstration in `R`. Try it out in your favourite programming language.
...
The agent is in particular interested in the *mean* of the variate $R$ in the full population, but has only access to the values of $R$ in a sample. How does the mean calculated from a sample of increasing size compare with the actual mean of the full population?

First let's create the values of the variates $X$ and $Y$, and construct $R$ from them:
...
R <- X/Y # variate considered by agent

## mean of R in the full population
popmean <- mean(R)
cat('\nThe full-population mean is', popmean, '(unknown to the agent)\n')
```

Now we imagine that the agent accumulates samples from the population, starting from 10, increasing by 100 units, until half of the population has been sampled. At each sample increase the agent calculates the sample mean. We plot how the sample mean changes with the sample size. We also plot indicate the full-population mean, which the agent doesn't know and is trying to guess:

```{r}
## sizes of successive samples
samplesizes <- seq(10, N/2, by=100)

## empty vector to contain the means of the increasing samples
...
for(sample in 1:length(samplesizes)){
    samplemeans[sample] <- mean(R[1:samplesizes[sample]])
}
...
tplot(x=samplesizes, y=samplemeans,
      ...
      ylim=c(-1,1)*1.01*max(abs(popmean),abs(samplemeans)))
abline(h=popmean, lty=2, lwd=4, col=7)
text(y=popmean, x=max(samplesizes), labels='population mean (unknown to agent)', adj=c(1,1), col=7, cex=1.2)
```

\

Now try a similar exercise but for the standard deviation of $R$
