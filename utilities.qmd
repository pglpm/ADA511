...
## Review of a basic decision problem: outcomes, actions, utilities {#sec-decisions-utilities}

Recall the structure of a basic decision problem:

![](basic_decision_tree.png){width=75%}

In order to choose an action, the agent needs:

- {{< fa cube >}} The set of possible [*actions*]{.yellow}, which we represent as sentences like $\yD$.

- {{< fa cube >}} The set of possible [*outcomes*]{.red}, whose truth is unknown to the agent. In the kind of decision problems that we are examining, the outcomes correspond to the possible values $\red Y\mo y$ of the [predictand]{.red} variate.


- {{< fa cube >}} The [*probabilities*]{.green} of the outcomes\ \ $\P({\red Y\mo y} \| \yD \and {\green X\mo x} \and \data \and \yI)$ which are determined by the agent's background information $\yI$, by any other available information, such as $\data$ about previously observed units and the values of some predictor variates $\green X\mo x$ for the current unit; and possibly also by the agent's action $\yD$ (see below).

- {{< fa cube >}} The [*utilities*]{.blue} of each pair of action and  outcome $(\yD \and {\red Y\mo y})$.

Some texts call the joint pair $({\red Y\mo y} \and \yD)$, or equivalently $(\yD \and {\red Y\mo y})$, of an action and an outcome, a [**consequence**]{.blue}.
...
- We are not assuming any temporal or "causal" relationship between [actions]{.yellow} and [outcomes]{.red}: **our framework works independently of these relationships**. In some decision-making problems the outcomes happen *after* an action is taken, and may be "influenced" by it or not. In other decision-making problems the outcomes have already happened *before* an action is taken.
    
	In a transportation-choice problem, for instance, the outcome "[wet from rain]{.red}" may happen after we take the action "[go on foot]{.yellow}" rather than "[go by bus]{.yellow}". In an image-classification problem, on the other hand, the outcome "[true label is $\cat{cat}$]{.red}" was already determined before we took the action "[classify as $\cat{dog}$]{.yellow}".

\

- In connection with the warning above, in some problems the probabilities of the outcomes may depend on the action; that is, knowledge about the action is *relevant* to the knowledge about the outcome (see again [§@sec-info-chapter]):
    
	$$\P({\red Y\mo y} \| \yD \and {\green X\mo x} \and \data \and \yI)$$
	
	For instance, in the transportation-choice problem the probability of the outcome "[wet from rain]{.red}" is higher conditional on the action "[go on foot]{.yellow}" than on the action "[go by bus]{.yellow}".
	
	In many decision-making problems typical of machine learning, such as classification, information about the action is *irrelevant* to the information about the outcome (which usually has already happened), and we have
...
In what follows we shall consider problems, such as classification, where knowledge of the agent's action is irrelevant to the outcome's probability. We shall nevertheless keep the more general notation\ \ $\P({\red Y\mo y} \| \yD \and {\green X\mo x} \and \data \and \yI)$.
...
Decision Theory says that whenever an agent decides on an action among alternatives having heterogeneous aspects, then it is *implicitly* using *only one real number* to summarize and bring together all those aspects. If this weren't true, the agent would be deciding in an irrational way, which could even be exploited against the agent itself.
...
[**Utility**]{.blue} is the name we give to the real number that encodes together all heterogeneous desirabilities and gains of an outcome. The convention is that the higher the utility, the more desirable is the outcome.^[With the opposite convention we speak of *disutility* or *loss*.]

### Notation

We denote the utility of the outcome $\red Y\mo y$ following from action $\yD$ as

$$
\uu(\yD \and {\red Y\mo y} \| \yI)
$$

This notation reminds us that the utilities assigned by an agent depend not only on the outcome, but also on the action and on the agent's background information $\yI$. 

The utilities for all outcomes and actions can be encoded in a [**utility matrix**]{.blue} $\um$, having one row per action and one column per outcome:
...
### Utilities in sequences of apparently identical actions {#sec-utility-sequence}

Keep in mind that the background information $\yI$ also includes *previous actions and outcomes*, if the present action and outcome are part of a sequence. Because of this, **when a decision problem occurs again, the utility of a particular action and outcome may be different from the previous ones.** This will be discussed in [§@sec-decision-trees].

As an example, suppose someone proposes you a sequence of bets. A coin will be tossed several times; before each toss you must decide between $\yDh \defd{}$["bet on heads"]{.yellow} or $\yDt \defd{}$["bet on tails"]{.yellow}; if the outcome is the one you bet on, you win $ 1, otherwise lose $ 1. The utility of a $ 1-loss at the first toss might be $\blue -1$:

$$
\uu(\yDh_1 \and {\red C_{\black 1}\mo\cat{tail}} \| \yI) = \blue -1
$$

But the utility of a $ 1-loss at the second toss might be lower, say $\blue -1.1$, if preceded by a loss:

$$
\begin{aligned}
&\uu(\yDh_2 \and {\red C_{\black 2}\mo\cat{tail}} \| 
\yDh_1 \and {\red C_{\black 1}\mo\cat{tail}} \and
\yI) = \blue -1.1
\\&\text{\small whereas}
\\
&\uu(\yDh_2 \and {\red C_{\black 2}\mo\cat{tail}} \| 
\yDh_1 \and {\red C_{\black 1}\mo\cat{head}} \and
\yI) = \blue -1
\end{aligned}
$$
...
- Bet A: If the coin lands `tails`, you win $11 000; if `heads`, you lose $10 000.
- Bet B: If the coin lands `tails`, you win $1; if `heads`, you lose $1.

A naive application of the principle of maximal expected utility would say that your expected money win of bet A is $500, whereas the expected win of bet B is $0. So bet A should be chosen.
...
$$
\uu(\$x) = C\,\ln\frac{\$C + \$x}{\$C}
$$

where $\$C$ is the total money capital, or a fraction thereof, owned by a person. When $x$ is very small compared to $C$, we have that\ \ $\uu(\$x) \approx x$.\ \ But when the magnitude of $x$ becomes comparable to $C$, the utility is no longer simply equal to $x$; losses are amplified, and wins are devalued. A loss of $\$C$ has a utility [of $-\infty$.]{.m}

Let's apply this formula. Suppose that a person owning a capital $\$C = \$20 000$ is offered the two bets above. The utilities of the various wins and losses are

$$
\begin{aligned}
\uu(\$11 000) &\approx 8765
\\
\uu(-\$10 000) &\approx -13 863
\\[1ex]
\uu(\$1) &\approx 1
\\
\uu(-\$1) &\approx -1
...
The expected utility of bet A is approximately $-2549$, and that of bet B is approximately $0$. Thus the principle of maximal expected utility leads to bet B as the optimal choice!

What's needed is not alternative principles, but a correct utility assessment. This is also a good place to recall the main theorem (by Wald, von Neumann and Morgenstern, Savage, and others) mentioned in [§@sec-decision-theory]: if one finds a different principle to take optimal action, then such an alternative principle is either (a) completely equivalent to maximal expected utility, for particular choices of utilities, or (b) logically inconsistent.

:::{.callout-caution}
According to the money-utility formula above, what's the lowest capital $\$C$ for which bet A becomes the optimal choice?
:::


### Continuous case

In some decision-making problems the set of possible actions can be considered as continuous.

A power-plant operator, for example, may have to decide to supply an amount of power between 100 MW and 200 MW to a geographical region in the next hour. The unknown "outcome" $\red Y$ in this scenario may be the power demand, which could be in the same range. In this case we can represent an action by a statement $\yellow D\mo d$, such as

$$\yellow D\mo 175\,\mathrm{MW}$$

(where "$\mo$" obviously stands for "set to", not for "is observed to be equal to"; recall [§@sec-sentence-notation]?)
...
It can be quite difficult to assess the utilities of the actions and outcomes in a decision-making problems, because of reasons such as heterogeneity or uncertainty, discussed below. Yet, Decision Theory says that any action is either implicitly using such a number, or is sub-optimal or logically inconsistent. Moreover, the specification, at some level, of utilities not derived by further analysis is simply unavoidable -- just like the specification of some initial probabilities.
...
The assessment of utilities can also be affected by uncertainties, and therefore become an inference problem in itself. The utility of an outcome may depend on further actions and further outcomes, whose utilities in turn depend on further actions and outcomes. Our basic decision framework can in this case be applied repeatedly, as briefly discussed below. In the simplest case, if the agent is uncertain between utility values $\blue \um'$ with probability $p'$, and utility values $\blue \um''$ with probability $p'' = 1-p'$, then the utilities to use are the averages
...
The utilities of the consequences not only allow the agent to determine the optimal action, as we shall see in the next chapter. They also allow us to quantify how much utility an agent yielded in a concrete application or sequence of applications of a specific decision-making task.
...
Suppose that in the first instance of a decision-making task the agent chooses action $\yD_1$, and in that instance the outcome ${\red Y}_1$ turns out to be $\ry_1$ (this may be discovered a long time after the action was taken). The utility (possibly a loss) gained at that instance is then, by definition,
...
Each agent is then applied to these data: it is given the predictors ${\green X_{\black i}\mo x_{\black i}}$, and from these it will determine and possibly execute the optimal action $\yD_i$, for all $i$. The total or average utility yield generated by the agent is then given by the formula above.
...
Note that the utilities do *not* depend on the probabilities, exactly as required by the independence axiom. The principle of maximal expected utility leads to action $\sA$, the gambler's favourite. Also note that the relevant probabilities are not the ones (about money winnings) mentioned in the initial statement of the problem. Just because we read or hear the word "probability" doesn't mean that that's the probability we need.
