...
### Output weights are *not* probabilities!

Some machine-learning algorithms are capable of giving a continuous "score" or "weight" between 0 and 1, instead of a simple "guessed unknown" answer. But unfortunately, typically **such weights are not probabilities or frequencies**, even if there may be some association between the weight and the probability or frequency.

A real example of this mismatch is evident in the following plot. It shows the 0–1 output weight given by a random forest in a binary-classification task (for drug discovery) on a test set, versus the actual frequency of the corresponding class:
...
Imagine what would happen if the clinician from our example above *mistook* the output weight for a probability:

- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output weight in the [range $0$--$0.25$]{.midgrey}: the clinician would choose treatment $\yA$. Luckily the upper boundary of this weight range approximately corresponds to a frequency of $0.25$, so in this case the clinician would be choosing the optimal treatment.

- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output weight in the [range $0.25$--$0.5$]{.midgrey}: the clinician would choose treatment $\yB$. Luckily the upper boundary of this weight range approximately corresponds to a frequency of $0.75$ (note the mismatch), so in this case the clinician would be choosing the optimal treatment.

- [{{< fa regular frown >}}]{.red}\ \ Output weight in the [range $0.5$--$0.75$]{.midgrey}: the clinician would choose treatment $\yB$. But the corresponding frequency is approximately between $0.75$--$0.9$, so the optimal treatment is actually $\yC$. As calculated in Scenario 3 above, the clinician would be then be shortening the patient's life by 7 months more than was unavoidable.

- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output weight in the [range $0.75$--$1$]{.midgrey}: the clinician would choose treatment $\yC$. Luckily this corresponds to an approximate frequency range $0.9$--$0.92$, where treatment $\yC$ is indeed optimal.


Note that we cannot say "the suboptimal treatment is chosen one out of four times", because in this example we don't know how many patients end up having a weight in the [$0.5$--$0.75$]{.midgrey} range. In a concrete case it could be that 90% of the patients end up in this range, which would mean that the misuse of the output weight would lead to a suboptimal treatment in 90% of cases.

:::{.callout-important}
## {{< fa exclamation-triangle >}} Don't use misleading terminology

Please don't call the score or weight output of a machine-learning algorithm a "probability", unless you've first made sure that it actually is a probability. Just because some numbers are positive and sum up to one doesn't mean that they are a probability distribution.

Using this kind of mistaken terminology shows downright **scientific incompetence**, and its consequences are borderline **scientific malpractice**.
:::
