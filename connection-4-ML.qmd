...
For instance, in a binary-classification with classes $\yca$ and $\ycb$, the application of two algorithms $\mA$ and $\mB$  to the same 100 test units could result in the following confusion matrices:
...
Yet, when put to actual use, it turned out that $\mB$ actually leads to a monetary *loss* of 3.5 $ per unit, whereas $\mA$ leads to a monetary *gain* of 3.5 $ per unit!
...
This means that for every test unit of class $\yca$ that is classified as $\yca$, the algorithms yields $\blue15 \$$; for every test unit of class $\yca$ that is classified as $\ycb$, the algorithms yields $\blue-35 \$$; and so on. The total yield of each algorithm for the 100 test units can therefore be calculated by multiplying the four utilities above by the corresponding counts in the algorithm's confusion matrix, and then taking the total. The average yield per unit is obtained dividing the total by the number of units, or directly using the normalized confusion matrix:
