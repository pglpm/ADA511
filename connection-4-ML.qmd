...
Recall ([§ @sec-basic-elements-inference]) that in probability notation $\P({\red\boldsymbol{\dotsb}}\|{\green\boldsymbol{\dotsb}} \and \yI)$, the [proposal]{.red} contains what the agent's belief is about, and the [conditional]{.green} contains what's supposed to be known to the agent, together with the background [information $\yI$.]{.m}
...
- [*An agent that can perform one of the tasks above also can, in principle, perform all other tasks*]{.blue}
...
- [*The probability distribution above encodes the agent's background knowledge and assumptions; different agents differ only in the values of that distribution.*]{.blue}
