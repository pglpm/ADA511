...
- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output score in the [range $0$--$0.25$]{.midgrey}: the clinician would choose treatment $\yA$. Luckily the upper boundary of this score range approximately corresponds to a frequency of $0.25$, so in this case the clinician would be choosing the optimal treatment.

- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output score in the [range $0.25$--$0.5$]{.midgrey}: the clinician would choose treatment $\yB$. Luckily the upper boundary of this score range approximately corresponds to a frequency of $0.75$ (note the mismatch), so in this case the clinician would be choosing the optimal treatment.

- [{{< fa regular frown >}}]{.red}\ \ Output score in the [range $0.5$--$0.75$]{.midgrey}: the clinician would choose treatment $\yB$. But the corresponding frequency is approximately between $0.75$--$0.9$, so the optimal treatment is actually $\yC$. As calculated in Scenario 3 above, the clinician would be then be shortening the patient's life by 7 months more than was unavoidable.

- [{{< fa regular face-grin-beam-sweat >}}]{.yellow}\ \ Output score in the [range $0.75$--$1$]{.midgrey}: the clinician would choose treatment $\yC$. Luckily this corresponds to an approximate frequency range $0.9$--$0.92$, where treatment $\yC$ is indeed optimal.
...
We can observe an even worse mismatch that in the random-forest case. For instance, if the softmax is between $0$--$0.25$, then the output-layer weight must have been less than approximately $-0.5$, which in turn means that the actual frequency could be anywhere between $0$ and $0.5$.
...
Please don't call the score or weight output of a machine-learning algorithm a "probability" or "frequency", unless you've first made sure that it actually is a probability or frequency. Just because some numbers are positive and sum up to one doesn't mean that they are a probability distribution.

Using this kind of mistaken terminology shows downright [**scientific incompetence**]{.purple}, and its consequences are borderline [**scientific malpractice**]{.purple}.
:::
...
## Evaluation metrics: the good and the bad {#sec-eval-metrics}
