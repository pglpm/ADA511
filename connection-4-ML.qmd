# [Beyond machine learning]{.red} {#sec-4-connection-ML}
{{< include macros.qmd >}}
...
The last few chapters gave a brief introduction to and overview of popular machine-learning methods, their terminology, and the typical points of view they adopt. Now let's try to look at them keeping in mind our main purpose in this course: [exploring new methods, understanding their foundations, and thinking out of the box](index.html).

For our purpose, the common machine-learning categorization and terminology discussed in [chapter @sec-ml-introduction] are somewhat inadequate. Distinctions such as "supervised learning" vs "unsupervised learning" are of secondary importance to a data engineer (as opposed to a ["data mechanic"](preface.html)) for several reasons:
...
### Interacting agents and strategies
...
- Chapters 16--18 in [*Artificial Intelligence*](https://hvl.instructure.com/courses/25074/modules/items/660089)

- [*Games and Decisions*](https://hvl.instructure.com/courses/25074/modules/items/720956)
:::
::::

We consider tasks where multiple "instances" with similar characteristics are involved, and the agent has some task related to a "new instance", possibly to be repeated an indefinite number of times.
...
Recall ([§ @sec-basic-elements-inference]) that in the probability notation $\P({\red\boldsymbol{\dotsb}}\|{\green\boldsymbol{\dotsb}} \and \yI)$, the [proposal]{.red} contains what the agent's belief is about, and the [conditional]{.green} contains what's supposed to be known to the agent, together with the background information $\yI$.
...
In the example above about image classification, a pictorial representation of this probability would be
...
In this task, the sentence $\red Z_{N+1}\mo z$ represents not the hypothesis that a *given* new instance has value $\red z$, but the possibility of *generating* a new instance with that value.
...
A remarkable trait of all the probabilities discussed in the above task categorization, even those for "hybrid" types of task, is that they can all be calculated from *one* probability distribution. We discussed and used this trait in [chapter @sec-learning].

A conditional probability such as $\P({\red\boldsymbol{\dotsb}}\|{\green\boldsymbol{\dotsb}} \and \yI)$ can always be written, by the `and`-rule, as the ratio of two probabilities:

$$
\P({\red\boldsymbol{\dotsb}}\|{\green\boldsymbol{\dotsb}} \and \yI)
=
\frac{
\P({\red\boldsymbol{\dotsb}}\and {\green\boldsymbol{\dotsb}} \| \yI)
}{
...
$$
\P({\red\boldsymbol{\dotsb}}\and {\green\boldsymbol{\dotsb}} \| \yI) =
\sum_{\yellow a}\P({\yellow A\mo a} \and {\red\boldsymbol{\dotsb}}\and {\green\boldsymbol{\dotsb}} \| \yI)
$$

And using this we find these final expressions for the probabilities above:
...
\sum_{{\purple z}, \yellow y_{N}, \dotsc, y_{1}}
\P(\red
Y_{N+1}\mo y
\black\and
\green
