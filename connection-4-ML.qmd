...
In many important tasks there's only a *statistical* relationship between predictands and predictors. "Statistical" means that whenever a predictor has value $\green X\mo x$, the predictand value may out to be $\green Y\mo y'$ in some units, but also $\green Y\mo y''$ in some other units, and so on. As a simple example, consider the [Norwegian population in 2022](https://www.ssb.no/befolkning/folketall/statistikk/befolkning/artikler/slik-ser-befolkningen-i-norge-ut). If our predictand is *sex* and we take as predictor that a person's *age* is between `85--89 years`, then a proportion $43 542/(43 542 + 28 220) \approx 61\%$ of those persons have *sex*=`female`, and the remaining $39\%$ proportion has *sex*=`male`. This doesn't mean that, say, `female` is the "true" output, and `male` is just the effect of noise, or vice versa. That would be nonsense.
...
The clinician has three treatments available: $\yA$, $\yB$, $\yC$. They have different intensities, which make them more or less effective against the disease, if present, but also less or more damaging to the patient if the disease is not present. Suppose that their efficacy and damage can be measured as the decrease in life expectancy for the patient. The effects are as follows:
...
- treatment $\yA$ is mild (this could be the no-treatment option): under it, the patient is expected to live four years shorter if the disease is present, but the life expectancy is unaltered if the disease is not present
...
The best decision is treatment $\yB$. A clinician who chose treatments $\yA$ or $\yC$ for patients having the same predictors as the present patient would on average *reduce* the life expectancy of each patient by one extra year.
...
Consider the vertical line with probability 0.1. Any strategy mixing treatment $\yA$ with any of the other two can only increase the reduction in expected-life.
...
- Does the output `yes` means that the probability for $\dise\mo\yy$ was above 50%? Then the clinician doesn't know whether it's above or below 75%, and can't make the optimal choice between $\yC$ and $\yB$.
- Is the output `yes` produced with a particular probability? But what is this probability? The output could be $\yy$ even if the probability were less than 50% or 25%. Then neither in this case can the clinician make the optimal choice between the three treatments.
