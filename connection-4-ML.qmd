...
Most evaluation metrics for classification combine the entries of a confusion matrix in a mathematical formula that yields a single number. Such metrics and formulae can be quite opaque. Their definitions, and the motivations for their introduction, are often arbitrary or very case-specific. It's common to find works where several metrics are used, because it's unclear which one should be used. And the only hope is that most or all of them will agree at least on the rankings they lead to.

Here are the scores that some popular metrics assign to algorithms $\mA$ and $\mB$, based on the confusion matrices above. The better algorithm according to each metric is in [**blue bold**]{.blue}:
...
| *metric*                           | $\mA$               |  $\mB$              |
+------------------------------------+---------------------+---------------------+
| Accuracy                           | 0.62                |  [**0.75**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| Precision                          | 0.64                |  [**0.70**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| Balanced Accuracy                  | 0.62                |  [**0.75**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| $F_1$ measure                      | 0.59                |  [**0.77**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| Matthews Correlation Coefficient   | 0.24                |  [**0.51**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| Fowlkes-Mallows index              | 0.59                |  [**0.78**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| True-positive rate (recall)        | 0.54                |  [**0.86**]{.blue}  |
+------------------------------------+---------------------+---------------------+
| True-negative rate (specificity)   | [**0.70**]{.blue}   |  0.64               |
+------------------------------------+---------------------+---------------------+
: {.sm}
...
Almost all these evaluation metrics seem to agree on $\mB$ being better. Yet, when put to actual use, $\mB$ actually leads to a monetary *loss* of 3.5 $ per unit, whereas $\mA$ leads to a monetary *gain* of 3.5 $ per unit!

Decision Theory tells us why, and indeed can predict it from the confusion matrices above.
