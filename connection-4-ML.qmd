# [Decisions: limitations of present-day machine-learning algorithms]{.blue} {#sec-ML-utility-limitation}
{{< include macros.qmd >}}
...
The book [*Machine Learning*](https://hvl.instructure.com/courses/25074/modules/items/661037) and Part V of the book [*Information Theory, Inference, and Learning Algorithms*](https://hvl.instructure.com/courses/25074/modules/items/660092) discuss interpretations of several machine-learning algorithms from the point of view of Probability Theory, and in few cases also of Decision Theory.
:::
...
An important reason for this distinction is that the set of possible decisions often does **not** have a correspondence with the set of unknown possibilities. In fact, the two sets often have different number of elements. You can think of many situations in which you are unsure whether some event will happen or not, and you have *three* plans: one if you are almost sure the event will happen, one if you are almost sure the event will not happen, and a third "safe" plan if you are around 50%/50% uncertain. The "safe" plan typically has consequences that are neither too bad or too good, so that losses and gains are kept to a minimum. This example also shows why the probabilities of the outcomes are important, which is the reason Decision Theory takes them into account.
...
For example suppose the clinician has three treatments available: $\yA$, $\yB$, $\yC$. They have different intensities, which make them more or less effective against the disease if present, but also less or more damaging to the patient if the disease is not present. Suppose that their efficacy and damage can be measured as the decrease in life expectancy for the patient. The effects are as follows:
...
In this scenario, a clinician that doesn't choose the optimal treatment is on average taking away from each patient between 7 months a 3 years of life more than was unavoidable
...
In this scenario, like in the first, a clinician that doesn't choose the optimal treatment is on average taking away between 7 months a 3 years of life more than was unavoidable, from each patient.
...
Some machine-learning algorithms are capable of giving a continuous "score" or "weight" between 0 and 1, instead of a simple "guessed unknown" answer. But unfortunately, typically **such scores are not probabilities or frequencies**, even if there may be some association between the weight and the probability or frequency.

A real-life example of this mismatch is evident in the following plot (see "for the extra curious" below for references). It shows the 0–1 output score given by a random forest in a binary-classification task (for drug discovery) on a test set, versus the actual frequency of the corresponding class:
...
## Evaluation practices and decision theory {#sec-eval-decision}

### Confusion matrices and confusing evaluation metrics
...
Such metrics and formulae can be quite opaque. Their definitions, and the motivations for their introduction, are often arbitrary or very case-specific. It's common to find works where several metrics are used, because it's unclear which one should be used. And the only hope is that most or all of them will agree at least on the rankings they lead to.

#### Majority votes are not a criterion for correctness

Here are the scores that some popular metrics assign to algorithms $\mA$ and $\mB$, based on the confusion matrices above. The better algorithm according to each metric is in [**green bold**]{.blue}:
...
Almost all these evaluation metrics seem to agree on $\mB$ being better.
...
These values may be determined by the combination of sale revenue, disposal costs, warranty refunds, and similar factors.

The utility matrix above says that for every test unit of true class $\yca$ (will function for at least a year) that is classified as $\yca$ (and therefore sent for sale) by the algorithms, the production company gains $\blue15 \$$; for every test unit of class $\yca$ that is classified as $\ycb$ (and therefore discarded) by the algorithms, the production company "gains" $\blue-35 \$$; and so on. The total yield from each algorithm on the 100 test units can therefore be calculated by multiplying the four utilities above by the corresponding counts in the algorithm's confusion matrix, and then taking the total. The average yield per unit is obtained dividing the total by the number of units, or directly using the normalized confusion matrix:
...
This calculation tells us that $\mA$ is better, and also gives us an idea of the actual utility it would produce.
...
::: {.callout-warning}
## {{< fa book >}} Study reading
[*Severe Class Imbalance: Why Better Algorithms Aren't the Answer*](https://hvl.instructure.com/courses/25074/modules/items/731407)
:::

### Popular evaluation metrics: the good and the bad
...
- Skim through [*Decision processes in perception*](https://hvl.instructure.com/courses/25074/modules/items/731180)

- Skim through [*Basic principles of ROC analysis*](https://hvl.instructure.com/courses/25074/modules/items/731179)
