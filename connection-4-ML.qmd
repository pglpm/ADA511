...
This means that for every test unit of class $\yca$ that is classified as $\yca$, the algorithms yield $\blue15 \$$; for every test unit of class $\yca$ that is classified as $\ycb$, the algorithms yield $\blue-35 \$$; and so on. The total yield of each algorithm for the 100 test units can therefore be calculated by multiplying the four utilities above by the corresponding counts in the algorithm's confusion matrix, and then taking the total. The average yield per unit is obtained dividing the total by the number of units, or directly using the normalized confusion matrix:
