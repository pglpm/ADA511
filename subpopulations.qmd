...
- Calculate the (marginal) frequency distribution for the $\vCa$ variate for the glass-fragment population of [table @tbl-glass-simple], considering this variate to have values $\cat{low}$ and $\cat{medium}$ (so we disregard $\cat{high}$). Is one value more frequent than the other?
...
| $\osuccess$ | $\oold$ | $\oonsite$ |
| $\ofailure$ | $\onew$ | $\oonsite$ |
: Reparations (each row is one unit, representing 100 reparations) {#tbl-reparations .sm .striped}

These population data are stored in the file [`repair_data.csv`](datasets/repair_data.csv).
...
- §§ 2.2--2.4 and 2.7--2.10 of [*Risk Assessment and Decision Analysis with Bayesian Networks*](https://hvl.instructure.com/courses/25074/modules/items/664427)

- [*The role of exchangeability in inference*](https://hvl.instructure.com/courses/25074/modules/items/660093)
