...
we notice that all units have variate **Ca** equal to `low`. In terms of conditional frequencies, this means
...
It is therefore impossible to observe other values of **Ca** in this new population.^[We are not claiming that this fact will be true if new units are considered; this important question will be discussed later.]
...
and so on. The reverse is therefore *not* true: if **Ca** is equal to `low`, that does *not* mean that it's impossible to observe other **Type** values besides `headlamps`. Note especially how these probabilities differ:
