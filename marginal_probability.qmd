...
:::{.callout-caution}
## {{< fa user-edit >}} Exercise
The points for the scatter plot of [fig. @fig-scatterXY] ([§ @sec-repr-joint-dens]) are saved in the file [`scatterXY_samples.csv`](exercise_data/scatterXY_samples.csv). Use them to represent the marginal probability density $\p(Y\mo y \| \yI)$, for the other quantity $Y$, as a scatter plot.
:::
