...
What preposition should follow the *noun* "right", as in "the right..."?

```{r}
probs <- infer(agent = opmSLM,
    predictand = 'word3',
    predictor = list(word2 = 'RIGHT'))

sort(probs, decreasing = TRUE)[1:5]
```

The `opmSLM` agent believes at 93% that it should be followed by the preposition "to".

Another grammar question: which is correct, "everyone has", or "everyone have"? Let's see the agent's beliefs about either sequence. To check them, we must use `infer()` to calculate all two-words probabilities, and then select those particular two words. We can compare the two probabilities against each other:

```{r}
probs <- infer(agent = opmSLM,
    predictand = c('word1', 'word2'))

probEveryoneHas <- probs['EVERYONE', 'HAS', drop = FALSE]

probEveryoneHave <- probs['EVERYONE', 'HAVE', drop = FALSE]

## probability of each, given that one of them is true

cbind(probEveryoneHas, probEveryoneHave) /
    c(probEveryoneHas + probEveryoneHave)
```
