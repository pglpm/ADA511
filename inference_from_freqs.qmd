...
Suppose we have three cubes in a row in front of us. They are marked #1, #2, #3. They look alike, but we know that two of them are labelled $\yy$ on their back, and one is labelled $\yn$ (or maybe they can be opened and have a labelled tag within). This background information -- let's call it $\yut$ -- is a simple case of a finite population with three units and a binary variate which we can call $C$. We know that the frequency distribution for this population is
...
$$f(C\mo\yy) = 2/3 \qquad f(C\mo\yn) = 1/3$$
...
\P(C_{1}\mo\yy \and 
C_{2}\mo\yy \and 
C_{3}\mo\yn \| F\mo\vf \and \yut)
+{}
\\&\qquad
\P(C_{1}\mo\yy \and 
C_{2}\mo\yn \and 
C_{3}\mo\yy \| F\mo\vf \and \yut)
+{}
\\&\qquad
\P(C_{1}\mo\yn \and 
C_{2}\mo\yy \and 
...
Now let's find the probability that a cube, say #1, is labelled $\yy$, given that we haven't observed any other cubes: $\P(C_1\mo\yy \| F\mo\vf \and \yut)$. It's a marginal probability ([§ @sec-marginal-probs]), so it's given by the sum

$$
\begin{aligned}
\P(C_1\mo\yy \| F\mo\vf \and \yut) &=
\sum_{i=\yy}^{\yn}\sum_{j=\yy}^{\yn}
\P(C_1\mo\yy \and C_2\mo i \and C_3\mo j \| F\mo\vf \and \yut)
\\[1ex]
&=
\P(C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yy \| F\mo\vf \and \yut) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yn \| F\mo\vf \and \yut) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yn \and C_3\mo\yy \| F\mo\vf \and \yut) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yn \and C_3\mo\yn \| F\mo\vf \and \yut)
\\[1ex]
&= 0 + 1/3 + 1/3 + 0
\\[1ex]
&= 2/3
...
which is indeed equal to $f(C\mo\yy)$.
...
- Calculate $\P(C_2\mo\yy \| F\mo\vf \and \yut)$, that is, the probability that cube #2 is labelled $\yy$, given that we don't know the label of any other cube. Is it different from $\P(C_1\mo\yy \| F\mo\vf \and \yut)$, or not? Why?

- Build a similar proof for a slightly different case; for example two cubes and three possible labels, or four cubes and two possible labels.
...
\P(C_{1}\mo\yy \and 
C_{2}\mo\yy \and 
C_{3}\mo\yn \| F\mo\vf \and \se{K}_{\text{NE}})
\\
\P(C_{1}\mo\yy \and 
C_{2}\mo\yn \and 
C_{3}\mo\yy \| F\mo\vf \and \se{K}_{\text{NE}})
\\
\P(C_{1}\mo\yn \and 
C_{2}\mo\yy \and 
C_{3}\mo\yy \| F\mo\vf \and \se{K}_{\text{NE}})
...
$$
    ...
    in such a way that they still sum up to $1$. Then find by marginalization the probability that cube #1 is labelled $\yy$. Is this probability still equal to the relative frequency of $\yy$?
:::


## Inference about several units {#sec-moreunit-freq-known}

Let's continue with the urn & cubes example of the previous section. We found that the probability that cube #1 is labelled $\yy$ was $2/3$, given that we haven't observed any other cubes. This probability was equal to the frequency of $\yy$-cubes in the urn.

Now suppose that we observe cube #2, and it turns out to be $\yy$. What is the probability that cube #1 is labelled $\yy$?

The probability we are asking about is $\P(C_{1}\mo\yy \| C_{2}\mo\yy \and F\mo\vf \and \yut)$, and it can be calculated with the usual rules. The result is again the same as the frequency of the $\yy$ cubes, but *with respect to the new situation*: there are now two cubes left in front of us, and one must be labelled $\yy$, the other $\yn$. The probability is therefore $1/2$, a value different from that we found before, $2/3$:

$$
\P(C_{1}\mo\yy \| F\mo\vf \and \yut) = 2/3
\qquad
\P(C_{1}\mo\yy \| C_{2}\mo\yy \and F\mo\vf \and \yut) = 1/2
$$

This situation is quite general: in a collection of many cubes, the probabilities for new observations change accordingly to information about previous observations (and also subsequent ones, if already known).

\

But consider now the case $\yul$ of a large collection of 3 000 000 cubes, 2 000 000 of which are labelled $\yy$ and the rest $\yn$.^[Note how this scenario becomes very similar to that of coin tosses.] The population's relative frequencies are exactly as in the case with three cubes, and for the probability that cube #1 is labelled $\yy$ we still have

$$
\P(C_1\mo\yy \| F\mo\vf \and \yul) = 
f(C\mo\yy) = \frac{2 000 000}{3 000 000} = 2/3
$$

Now suppose we observe cube #2 and see label $\yy$. What is the probability that cube #1 is labelled $\yy$? In this case we find

$$
\P(C_1\mo\yy \| C_2\mo\yy \and F\mo\vf \and \yul) = 
\frac{1 999 999}{2 999 999} \approx 2/3
$$

with an absolute error of only $0.000 000 1$. That is, the probability and frequency are almost the same as before observing cube #2. The reason is clear: the number of cubes is so large that observing some of them doesn't practically change the content and proportions of the collection.

The joint probability that cube #2 is $\yy$ and cube #1 is $\yy$ is therefore, by the `and`-rule,

:::{.column-page-inset-right}
$$
\begin{aligned}
\P(C_2\mo\yy \and C_1\mo\yy \| F\mo\vf \and \yul) &=
\P(C_1\mo\yy \| C_2\mo\yy \and F\mo\vf \and \yul) \cdot
\P(C_2\mo\yy \| F\mo\vf \and \yul)
\\[1ex]
&\approx f(C\mo\yy) \cdot f(C\mo\yy)
\end{aligned}
$$
:::

the approximation being the better, the larger the collection of cubes.

It is easy to see that this will hold for more observations, and for different and more complex labels, as long as the number of cubes considered is enough small compared with the population size. For instance

:::{.column-page-inset-right}
$$
\P(C_4\mo\yy \and C_3\mo\yn \and C_2\mo\yn \and C_1\mo\yy \| F\mo\vf \and \yul)
\approx 
f(C\mo\yy) \cdot f(C\mo\yn) \cdot f(C\mo\yn) \cdot f(C\mo\yy)
$$
:::
...
&\qquad{}=\frac{
f(\bZ_{u}\mo {\blue z}) \cdot
\cancel{f(\bZ_{u'}\mo {\blue z'})} \cdot 
\cancel{f(\bZ_{u''}\mo {\blue z''})} \cdot 
\cancel{f(\bZ_{u'''}\mo {\blue z'''})} \cdot
\cancel{\dotsb}
}{
\underbracket[0.2ex]{\sum_{\lblue z}
f(\bZ_{u}\mo {\blue z})}_{{}=1} \cdot
\cancel{f(\bZ_{u'}\mo {\blue z'})} \cdot 
\cancel{f(\bZ_{u''}\mo {\blue z''})} \cdot 
\cancel{f(\bZ_{u'''}\mo {\blue z'''})} \cdot
\cancel{\dotsb}
...
Note that there is always learning about something whenever we put new information in the conditional of a probability. In the cube-collection example above, for example we have

$$
\P(C_{1} \mo \yy \|  \yul) = 2/3
\qquad
\P(C_{1} \mo \yy \| C_{2} \mo \yn \and \yul) = 2/3
$$

and the agent has (practically) not learned anything [about the sentence $C_1\mo\yy$]{.purple} from the sentence $C_2\mo\yn$.

But we also have

$$
\P(C_{2} \mo \yy \|  \yul) = 2/3
\qquad
\P(C_{2} \mo \yy \| C_{2} \mo \yn \and \yul) = 0
$$

the probability for the sentence $C_2\mo\yy$ has changed. So the agent has learned something: that cube #2 is labelled $\yn$.
:::
