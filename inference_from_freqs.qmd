...
Suppose we have three cubes in a row in front of us. They are marked #1, #2, #3. They look alike, but we know that two of them are labelled $\yy$ on their back, and one is labelled $\yn$ (or maybe they can be opened and have a labelled tag within). This background information $\se{K}$ is a simple case of a finite population with three units and a binary variate which we can call $C$. We know that the frequency distribution for this population is
...
Given $F\mo\vf$, we know that $F\mo\vf$ is true: $\P( F\mo\vf \| F\mo\vf \and \se{K})=1$, which means
...
C_{2}\mo\yy \and 
C_{3}\mo\yy)
\|[\big] F\mo\vf \and \se{K}\bigr]
\\[2ex]
&=
\P(C_{1}\mo\yy \and 
C_{2}\mo\yy \and 
C_{3}\mo\yn \| F\mo\vf \and \se{K})
+{}
\\&\qquad
\P(C_{1}\mo\yy \and 
C_{2}\mo\yn \and 
C_{3}\mo\yy \| F\mo\vf \and \se{K})
+{}
\\&\qquad
\P(C_{1}\mo\yn \and 
C_{2}\mo\yy \and 
...
According to our background information $\se{K}$, our degrees of belief are exchangeable. This means that the three probabilities summed up above must all have the same value, because in each of them $\yy$ appears twice and $\yn$ once. But if we are summing the same value thrice, and the sum is $1$, that that value must be $1/3$. Hence we find that
...
Now let's find the probability that a cube, say #1, is labelled $\yy$, given that we haven't observed any other cubes: $\P(C_1\mo\yy \| F\mo\vf \and \se{K})$. It's a marginal probability ([§ @sec-marginal-probs]), so it's given by the sum

$$
\begin{aligned}
\P(C_1\mo\yy \| F\mo\vf \and \se{K}) &=
\sum_{i=\yy}^{\yn}\sum_{j=\yy}^{\yn}
\P(C_1\mo\yy \and C_2\mo i \and C_3\mo j \| F\mo\vf \and \se{K})
\\[1ex]
&=
\P(C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yy \| F\mo\vf \and \se{K}) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yn \| F\mo\vf \and \se{K}) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yn \and C_3\mo\yy \| F\mo\vf \and \se{K}) + {}
\\ &\qquad
\P(C_1\mo\yy \and C_2\mo\yn \and C_3\mo\yn \| F\mo\vf \and \se{K})
\\[1ex]
&= 0 + 1/3 + 1/3 + 0
\\[1ex]
&= 2/3
...
- Calculate $\P(C_2\mo\yy \| F\mo\vf \and \se{K})$, that is, the probability that cube #2 is labelled $\yy$, given that we don't know the label of any other cube. Is it different from $\P(C_1\mo\yy \| F\mo\vf \and \se{K})$, or not? Why?
...
But consider now the case $\yul$ of a [$\text{L}$arge]{.m} collection of 3 000 000 cubes, 2 000 000 of which are labelled $\yy$ and the rest $\yn$. The population's relative frequencies are exactly as in the case with three cubes, and for the probability that cube #1 is labelled $\yy$ we still have
