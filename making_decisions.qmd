...
:::{.callout-important}
## The final decision is *not* unpredictable

Note that even when we apply such ideas, **we are still nevertheless following the principle of expected-utility maximization**. We haven't changed or contradicted that principle in any way.
...
Unfortunately many texts oversimplify this idea, and seem to imply that in some cases it's best to make decisions "at random", in apparent contradiction with the principle of expected-utility maximization. As we have seen, this is not the case.
\
...
### The reason for unpredictably choosing among options with equal expected utility

In some decision-making problems, several choices may happen to have equal, maximal expected utilities. In such situations it is usually recommended to "choose randomly" among those choices. This recommendation is an application of the ideas discussed above.

The reason is that we could have made some small error in estimating utilities, so that the actual expected utility of one of those choices might be actually higher than the others'. If we simply decided to choose, say, the first listed option, then in many repetitions of this decision-making problem me might actually be incurring an accumulated loss, if the actual best one were, say, the second option.

If we redefine our set of decisions, instead, as

[#1]{.yellow}. Follow option 1.  
[#2]{.yellow}. Follow option 2.  
...  
[#0]{.yellow}. Use a random number generator to choose which of the options above to follow.

and in our decision problem include the possibility that the utilities have a small error, then a quick calculation shows that decision [#0]{.yellow} is the one with maximal expected utility.

:::{.callout-caution}

Try to draw the diagram for the redefined decision problem above. Include additional possible outcomes that take into account the possible errors in utilities, and use the redefined sets of decisions.

Show that, with this more precise diagram, decision [#0]{.yellow} above has indeed highest expected utility.
:::
