...
The reason is that we could have made some small error in estimating utilities, so that the exact expected utility of one of those choices might be actually higher than the others'. If we simply decided to always choose, say, the first listed option, then in many repetitions of this decision-making problem we might incur an accumulated loss, if the actual optimal option were, say, the second.
...
and in our decision problem include the possibility that the utilities have a small error, then a quick calculation shows that decision [#0]{.yellow} is the one with maximal expected utility in a repetition of this decision problem.

:::{.callout-caution}

Try to draw the diagram for the redefined decision problem above, repeated twice. Include additional possible outcomes that take into account the possible errors in utilities, and use the redefined sets of decisions.
