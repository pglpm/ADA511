...
The first inference problem seems -- and notoriously is -- more difficult than the second. But why?

Many texts would categorize the first inference as a "[time-series analysis](https://www.abs.gov.au/statistics/understanding-statistics/statistical-terms-and-concepts/time-series-data)", because it involves changes in time. Time cannot be the fundamental factor that makes the first inference more difficult, however, because the second inference involves time as well: we want to assess the failure status of a future component, given the status of past ones.
