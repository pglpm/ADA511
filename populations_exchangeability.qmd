...
$$
\P(X_1\mo x_1,\ X_2\mo x_2,\ X_3\mo x_3,\ \dotso \| \yI)
$$

for all combinations of values $x_1, x_2, x_3, \dotsc$


This joint probability distribution is called [**exchangeable**]{.blue} if [*all marginal probabilities ([§ @sec-marginal-probs]) for any sub-collection of quantities remain the same whenever the values are swapped*]{.blue}. For example:
...
\\[1ex] {}={}&
\P(X_1 \mo \yb,\ X_2\mo \ya\| \yI)
\end{aligned}
\qquad\text{\small and}\qquad
\begin{aligned}
&
\P(X_5 \mo \yc,\ X_{11}\mo \yd \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \yd,\ X_{11}\mo \yc\| \yI)
\end{aligned}
$$
...
\\[1ex] {}={}&
\P(X_1 \mo \ya,\ X_2\mo \yc,\ X_3\mo \yb \| \yI)
\\[1ex] {}={}&
\P(X_1 \mo \yb,\ X_2\mo \ya,\ X_3\mo \yb \| \yI)
\\[1ex] {}={}&
\P(X_1 \mo \yc,\ X_2\mo \yb,\ X_3\mo \ya \| \yI)
\\[1ex] {}={}&
\P(X_1 \mo \yc,\ X_2\mo \yb,\ X_3\mo \ya \| \yI)
\\[1ex] {}={}&
\P(X_1 \mo \yc,\ X_2\mo \ya,\ X_3\mo \yb \| \yI)
\end{aligned}
\qquad\text{\small and}\qquad
\begin{aligned}
&
\P(X_5 \mo \ya,\ X_{11}\mo \yb,\ X_{37}\mo \yc \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \ya,\ X_{11}\mo \yc,\ X_{37}\mo \yb \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \yb,\ X_{11}\mo \ya,\ X_{37}\mo \yb \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \yc,\ X_{11}\mo \yb,\ X_{37}\mo \ya \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \yc,\ X_{11}\mo \yb,\ X_{37}\mo \ya \| \yI)
\\[1ex] {}={}&
\P(X_5 \mo \yc,\ X_{11}\mo \ya,\ X_{37}\mo \yb \| \yI)
\end{aligned}
$$
...
\\[1ex] {}={}&
\P(X_1 \mo \yc,\ X_2\mo \ya,\ X_3\mo \yd,\ X_4\mo \yb \| \yI)
\\[1ex] {}={}&
\P(X_1 \mo \yd,\ X_2\mo \yc,\ X_3\mo \yb,\ X_4\mo \ya \| \yI)
\\[1ex] {}={}&\dotso
\end{aligned}
$$
...
[Note that the commas just stand for the `and` operation, so thanks to the commutativity of Boolean algebra ([§ @sec-boolean]) we could equivalently write
$$
\begin{aligned}
&
\P(X_1 \mo \ya,\ X_2\mo \yb\| \yI)
\\[1ex] {}={}&
\P(X_{\underbracket[0ex]{2}_{\uparrow}} \mo \ya,\ X_{\underbracket[0ex]{1}_{\uparrow}}\mo \yb \| \yI)
\end{aligned}
$$
...
This does not mean that *all* probabilities need to be equal. For instance, if $\yb\ne\yc$ then we can have the *in*equality
...
Consider the following marginal probability distributions. For each, determine whether the joint probability they come from is *surely **non**-exchangeable*, and explain why:

- $$\begin{aligned}
&\P(X_1\mo 3,\  X_4\mo 5) = 0.15
\\
&\P(X_1\mo 5,\  X_4\mo 3) = 0.15
\\
&\P(X_1\mo 7,\  X_4\mo 3) = 0.25
\\
&\P(X_1\mo 3,\  X_4\mo 7) = 0.25
\end{aligned}
$$

- $$\begin{aligned}
&\P(Y_1\mo 3,\  Y_4\mo 5) = 0.07
\\
&\P(Y_1\mo 5,\  Y_4\mo 3) = 0.07
\\
&\P(Y_9\mo 3,\  Y_{21}\mo 5) = 0.4
\\
&\P(Y_9\mo 5,\  Y_{21}\mo 3) = 0.4
\end{aligned}
$$

- $$\begin{aligned}
&\P(Z_1\mo \yhi,\  Z_7\mo \ylo,\  Z_{20}\mo \yhi) = 0.11
\\
&\P(Z_1\mo \yhi,\  Z_7\mo \yhi,\  Z_{20}\mo \ylo) = 0.35
\\
&\P(Z_2\mo \ylo,\  Z_3\mo \ylo,\  Z_9\mo \yhi) = 0
\\
&\P(Z_2\mo \yhi,\  Z_3\mo \ylo,\  Z_9\mo \ylo) = 0
\end{aligned}
$$

- $$\begin{aligned}
&\P(U_1\mo 6.4,\  U_2\mo 8.7,\  U_3\mo -0.3) = 0.1
\\
&\P(U_1\mo 1.7,\  U_2\mo 0.5,\  U_3\mo -0.3) = 0.1
\\
&\P(U_1\mo -0.3,\  U_2\mo 6.4,\  U_3\mo 0.5) = 0.01
\\
&\P(U_1\mo -0.3,\  U_2\mo 1.7,\  U_3\mo 0.5) = 0.4
\end{aligned}
$$

Note that we can't say that a probability distribution is exchangeable unless we're given *all* its probabilities, for all combinations of quantities and values.
:::
