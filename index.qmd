...
<!-- [**No Large Language Models were used in the preparation     -->
<!-- of these lecture notes and of their accompanying material and code.**]{style="text-align:center;font-size:75%"} -->
...
The goal of this course is not to help you learn how to do a logistic regression; or to tune the parameters of the latest kind of Machine-Learning algorithm; or to cross-validate the performance of a deep neural network; or to train a local Large Language Model.

The goal of this course is deeper and more long-sighted: you'll learn the principles of Data Science and inference, and how to build Machine-Learning algorithms and AI agents *of the future*. As a bonus you'll also learn how to concretely improve present-day algorithms, doing this concretely during the course.

Your acquired knowledge will cover the basics of the fields listed below. Be aware that the list may look heterogeneous, and you may have never heard of some of those fields. Yet, they are all different sides of the same thing:
...
The reason why these fields go hand-in-hand is simple. AI agents have to *observe*, to *reason*, and often to *guess*; and then they have to *take action*. But these are exactly the same things we humans must do when analysing data, drawing scientific inferences, and making decisions! No surprise really: that's why it's called "intelligence"; "artificial" because it's implemented in a robot rather than coming from a human. But by learning how to implement it in a robot, you're also learning how to do it yourself.

:::{.column-margin}
[![](atlas-boston_dynamics.jpg){width=100%}](https://bostondynamics.com/video/atlas-goes-hands-on/?wvideo=xcgevucvm1)
AI agents like Boston Dynamic's *Atlas* must observe, guess, act (click for video)
:::

This is the cool aspect of the disciplines above. If you study one of them from a particular angle, then you're also learning all the others -- for free! **This is what this course sets out to do.** So be ready for surprises: after learning some topics, the course will show you that you can apply what you just learned to completely different contexts.

This course goal will be achieve not in a "theoretical" way, but very much hands-on. In the second part of the course you will build, essentially from scratch, a "perfect" agent -- we'll call it an "Optimal Predictor Machine" -- capable of drawing inferences, making decisions, and even acting as a "small language model" in simplified situations. And you'll be able to trace step-by-step *why* the agent operates the way it does; no black boxes.

\

How can such a simultaneous-learning goal be achieved?

There is a small set of rules and one method that are *mathematically guaranteed* to output the optimal solution of any inference, guess, prediction, and decision-making problem -- by human or robot. You can think of this set as defining an "unbeatable, optimal universal machine". Or, from an AI point of view, you can think of these rules and method as the "laws of artificial intelligence" that should govern any ideal AI designed to draw inferences, give answers, and make decisions.
...
\\[1ex]
&\green\texttt{\small choose }\ \argmax\limits_{\se{A}}\ \sum_{\se{Y}}\mathrm{U}(\se{Y} \land \se{A}\| \se{X})\cdot \P(\se{Y} \| \se{A} \land\se{X})
\end{aligned}
$$]{style="font-size:60%;"}
:::
...
- The [**utility** part (top-right, light-blue box)]{.lightblue} develops the "decision/action engine" of the machine. Here you will meet several ideas that will probably be quite new to you -- but also very simple and intuitive.
...
We shall start with a quick preview of the [**solution**]{.blue} part in chapters [-@sec-intro]--[-@sec-basic-decisions], because it is very simple to understand, and it shows why the *inference* and the *utility* parts are necessary.

Then we shall continue with the [**inference**]{.green} parts in chapters 5--10, 14--18, 24--29, alternating them with the [**data**]{.yellow} parts in chapters 12--13, 20--23, and with interludes about [**present-day machine-learning**]{.midgrey} in chapters 4, 11, 19. Quick [introductions to the R programming language]{.lightblue} are also given at appropriate places.

<!-- 
Then we shall continue with the [**inference**]{.green} parts in chapters [-@sec-what-inference]--[-@sec-monty], [-@sec-prob-distribs]--[-@sec-info-chapter], [-@sec-kinds-inferences]--[-@sec-summary-formulae], alternating them with the [**data**]{.yellow} parts in chapters [-@sec-quantities-types-basic]--[-@sec-quantities-types-multi] and--[-@sec-variates]--[-@sec-pop-samples], and with interludes about [**present-day machine-learning algorithms and their approximations**]{.midgrey} in chapters [-@sec-connection-1-ML], [-@sec-connection-2-ML], [-@sec-connection-3-ML], [-@sec-ml-introduction]--[-@sec-eval-decision]. Quick [introductions to the R programming language]{.lightblue} are also given at appropriate places.

 -->
 
As soon as the [inference]{.green} and [data]{.yellow} parts are complete, you will be able to apply the machine to real, albeit not too complex, inference problems. This [**application**]{.red} will be made in chapters 30--34.

We finally round up with the [**utility**]{.blue} part in chapters 35--36, extending our concrete [**application**]{.red} to it in chapters 37--38. Final connections with [**present-day machine learning and its limitations**]{.midgrey} are made in chapters 39--43.
