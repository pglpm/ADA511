...
Some data scientists have in-depth knowledge on how to optimally store and read large amounts data; what kind of machine-learning algorithm should be used in a given task; how to fine-tune an algorithm's parameters, and the currently best software for that purpose. Their particular knowledge is fundamental for the working of today's technological infrastructure.
...
- identify which sub-optimal approximations to the fundamental rules are made by popular machine-learning algorithms

- exploit new technologies to build new algorithms that do calculations closer to the exact theoretical ones, thereby achieve a performance closer to the theoretical optimum

It is also possible that they don't know that there are only five of six fundamental laws upon which any inference, prediction, classification, regression, decision task is (or ought to be) based upon. Because this is a kind of knowledge and skills that they don't need.

Data scientists with this kind of knowledge are "data mechanics", to draw a parallel with the car example.

A "data engineer", on the other hand, is the kind of data scientist who has no difficulties with the knowledge and skills implicit in the bullet points above; but at the same time may not know what software to use for tuning parameters of a particular class of deep networks, or the best format to store particular kinds of data.
...
There is a plethora of academic courses, in all kinds of format, that target knowledge and goals for the "data mechanic". But those courses are usually inadequate to cover the knowledge and goals for the "data engineer". Some courses, misleadingly, even present approximations and recipes that are only valid for particular situations as if they were universal rules or methods instead. For example, if you have some acquaintance with machine learning through a "data mechanics" course, you might be surprised by the following facts:

- [{{< fa exclamation-circle >}}]{.midgrey}\ Splitting of data into "training set", "validation set", and similar sets, are *not* part of the exact application of the laws of inference and decision-making; in fact, they would lead to *sub-optimal* results. Rather, such splittings are done as coarse approximations of the exact application.

- [{{< fa exclamation-circle >}}]{.midgrey}\ Cross-validation and similar techniques are *not* part of the exact methods either. They also arise as approximations of the exact methods.

- [{{< fa exclamation-circle >}}]{.midgrey}\ It is possible to calculate, within probable bounds, the maximal accuracy (or other performance metric) achievable by *any* classification or regression algorithm for a given application.

- [{{< fa exclamation-circle >}}]{.midgrey}\ Some evaluation metrics, such as the [area under the curve of the receiver operating characteristic (AUC)](https://developers.google.com/machine-learning/crash-course/classification/roc-and-auc) have intrinsic flaws and may attribute higher values to worse-performing algorithms.
...
We believe that it is important to teach and keep alive the less "mechanic" and more "engineer" side of data science. Continuous advances in computational technology -- think of quantum computers -- may offer completely novel and superior ways to approximate the exact laws of inference and decision. But only the data scientists who know these laws and understand present-day approximations will be able to exploit new technologies. But already today several methods could be greatly optimized by any data engineer who has studied the basic principles.

The aspiration of the present course is to give such an introduction for "data engineers" as distinguished from "data mechanics".
