...
: There's only one true teacher in your life, and that's yourself, with your curiosities, passions, aspirations. We have tried to provide a lot of extra reading material for those who want to deepen their understanding of topics covered or just connected to the present course. Maybe you'll stumble into a new passion or even into your life call?
...
The course structure reflects the way in which the ideal universal decision-making machine works. It can be roughly illustrated as follows (this is just a caricature, don't take this diagram too literally):

```{mermaid}
flowchart TB
  C{{probability calculus}} --o Z[/max expected utility\]
  V{{utility}} --o Z
  B([sentences]) --> C
  A[quantity] --o B
%%  D[value] --o B
...
%%  style D fill:#cb4,stroke-width:0pt
  ...
  style B fill:#283,color:#fff
  style C fill:#283,stroke-width:3pt,color:#fff
  style S fill:#6ce,stroke-width:0pt
  style T fill:#6ce,stroke-width:0pt
  style Q fill:#6ce,stroke-width:0pt
  style V fill:#6ce,stroke-width:3pt
...
- [**Data**]{.yellow} parts develop the language in which any [**problem**]{.red} can be fed into the inference engine. Here you will also learn about important pitfalls in handling data.
