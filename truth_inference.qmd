...
Formal logic gives us a set of rules for correctly drawing sure inferences, *when sure inferences are possible*. These rules can be formulated in different ways, leading to a wide variety of [deduction systems](https://plato.stanford.edu/archives/spr2023/entries/natural-deduction), each one with a wide variety of possible notation conventions. These systems are all equivalent, of course.

[As an example, look at how a proof of our inference appears in the so-called *sequent calculus*. This inference calculus consists of a dozen or so inference rules. Note that this system adopts a notation different from ours: it uses the symbol "$\vdash$" instead of a conditional bar, and puts the *proposal on the right* and the *conditional on the left*, swapping our placement:]{.small}

![](failure_sequent_red.png){width=50% fig-align="center"}

[The [bottom formula]{.red} is the target inference. Each horizontal line denotes the application of an inference rule, from one or more inferences above the line, to one below the line. The two formulae with no line above are our [starting inference]{.green} and a tautology.]{.small}

::::{.column-margin}
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious

Takeuti: [*Proof Theory*](references.html)
:::
::::
...
- Ben-Ari: [*Mathematical Logic for Computer Science*](references.html)
- Pelletier & Hazen: [*Natural Deduction Systems in Logic*](https://plato.stanford.edu/archives/spr2023/entries/natural-deduction)
:::
::::
