...
$$
Above the line we write the sentences representing the data we have; or the data we imagine to have, if we're drawing a hypothetical inference. Below the line we infer the information that interests us.

How could we draw the obvious inference above? Which rules did we follow? Did we make any hidden assumptions, or use data that weren't explicitly mentioned?

Logic is a huge field that formalizes and makes rigorous the rules that a rational person or an artificial intelligence should use in drawing certain inferences. We'll get a glimpse of it here, as a trampoline for jumping towards the more general inferences that we need in data-driven engineering problems.

### Basic sentences

We start by writing down the _basic_^[A more technical term is "atomic"] sentences that constitute our data and that underlie the inferences we want to draw. "Basic" in the sense that we will not analyse these sentences into further sub-sentences. In the trivial example above we identify two such sentences: $\pr{My umbrella is blue}$, and $\pr{My umbrella is red}$. Let's represent them by symbols:
$$
\begin{aligned}
...
From this last remark we see that the sentence $\pr{My umbrella is either blue or red}$ does not really correspond to $b \lor r$, because the latter formula includes the possibility that my umbrella could be *both* blue *and* red. But we are implicitly assuming that this cannot happen; it's physically impossible. We have thus discovered that there another piece of data hidden in our inference: my umbrella cannot be both blue and red. Convince yourself that we can write this hidden data like this:
$$
\lnot(b \land r) \ .
$$

There's even more information implicitly understood in our data. In our case, for example, it must be understood whose umbrella we're speaking about, what an umbrella is, what "red" and "blue" mean, and so on. Usually this kind of data so obvious that we don't think about it very much and don't mention it. In real engineering problems, however, it may happen that we have to stop and examine this implicit knowledge, maybe to check whether there are contradictory data or goals. It's a good idea to represent this implicitly-understood knowledge by a symbol, for example $D$ (for "extra data"; we could have used $K$ for "knowledge" or $I$ for "information").
...
- Basic sentences:
    ...
    + $r=\pr{My umbrella is red}$.

    + $D={}$ (a long collection of sentences explaining all other information understood implicitly).

- Assumptions, explicit and implicit:

    + $b\lor r= \pr{My umbrella is either blue or red}$

    + $\lnot r = \pr{My umbrella is not red}$

    + $\lnot (b \land r) = \pr{My umbrella cannot be both blue and red}$; this is an implicitly understood fact.
...
Deduction systems in formal logic give us a set of rules for making correct inferences, that is, for correctly determining whether the conclusions of interest are true or false. These rules are represented in a [wide variety of ways](https://plato.stanford.edu/archives/spr2023/entries/natural-deduction) as steps leading from one conclusion to another one. Using the so-called sequent calculus, for instance, a proof of our inference would look like this^[Obtained with the online [Propositional sequent calculus prover](https://www.nayuki.io/page/propositional-sequent-calculus-prover).]:

![The top line is our inference; the lines below it represent steps in the proof. Steps with narrower indentation depend on steps with wider indentation. The two lines with widest indentation are *axioms*, that is, basic conclusions that we take as intuitively evident and don't need further proof. Those two lines basically say something trivial: "if we assume that my umbrella is blue, then we can conclude that it is blue, within that assumption", and analogously for "red".](umbrella_sequent_proof.png){width=50%}

Only particular steps are allowed -- these are the inference rules, about a dozen or so in number for sequent calculus. For example, one rule says that @@ 
\

We can compactly encode all these rules in the following way. First, represent `true` with the number `1`, and `false` with `0`. Second, if the conclusion $C$ given assumptions $A$ is true by writing
$$
\mathrm{T}(C \| A) = 1
$$
and that it is `false` according by writing
$$
\mathrm{T}(C \| A) = 0
$$

Then the rules of truth inference are summarized by the following equations, which must always hold for any sentences $A,B,C$, no matter whether they are basic or complex:
