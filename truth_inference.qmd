...
$$
and we shall take this to be `true` (that is, to have probability [$100\%$]{.together}).
...
Formal logic gives us a set of rules for correctly drawing sure inferences, *when such inferences are possible*. These rules can be formulated in different ways, leading to a wide variety of [deduction systems](https://plato.stanford.edu/archives/spr2023/entries/natural-deduction) (each one with a wide variety of possible notations). The picture on the margin, for instance, shows how a proof of how our inference would look like, using the so-called sequent calculus, which consists of a dozen or so inference rules.
...
First, represent `true` by the number [$\mathbf{1}$,]{.together} and `false` by [$\mathbf{0}$.]{.together}
...
The rules of truth-inference are then encoded by the following equations, which must always hold for any atomic or complex sentences [$\yX,\yY,\yZ$:]{.together}
...
Each equality can first be rewritten in different ways according to the usual rules of algebra. Then the resulting left side can be replaced by the right side, and vice versa. The numerical values of starting inferences can be replaced in the corresponding expressions.
::::
:::::
...
$$
{\color[RGB]{102,204,238}\tru(\yX \| \yY \land \yZ)}
=\color[RGB]{204,187,68}\frac{\tru(\yX \land \yY \| \yZ)}{\tru(\yY \| \yZ)}
$$
...
$$
\color[RGB]{238,102,119}\tru(\lnot\yh \| \ys \land \yf \land \yI)
$$
starting from the given ones
$$
\color[RGB]{34,136,51}\tru(\lnot\yh \lor \lnot\ys \| \yf \land \yI) = 1
\ ,
\qquad
\tru(\ys \| \yf \land \yI) \ne 0
$$
...
{\color[RGB]{34,136,51}\underbracket[1pt]{\tru(\ys \| \yf \land \yI)}_{\ne 0}}
...
&&\text{\small  ∧-rule}
\\
&\qquad=\frac{{\color[RGB]{34,136,51}\tru(\lnot\yh \land \lnot\ys \| \yf \land \yI)}-
...
:::
Therefore
[$\color[RGB]{238,102,119}\tru(\lnot\yh \| \ys \land \yf \land \yI) = 1$.]{.together}
...
:::{.callout-caution}
## {{< fa pen >}} Exercise
Prove the target inference $\color[RGB]{238,102,119}\tru(\lnot\yh \| \ys \land \yf \land \yI) = 1$ using the rules of truth-inference, but beginning from the starting inference [$\color[RGB]{34,136,51}\tru(\lnot\yh \land \lnot\ys \| \yf \land \yI)=1$.]{.together}
:::
...
::: {.callout-caution}
## {{< fa book >}} Reading
Look over Ch. 7 in [*Artificial Intelligence*](https://hvl.instructure.com/courses/25074/modules/items/660089).
:::
...
- [*Logic in Computer Science*](https://hvl.instructure.com/courses/25074/modules/items/661036)
- [*Mathematical Logic for Computer Science*](https://hvl.instructure.com/courses/25074/modules/items/661146)
