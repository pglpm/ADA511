...
$$
and we shall take this to be `true` (that is, to have probability [$100\%$]{.m}).
...
First, represent `true` by the number [$\mathbf{1}$,]{.m} and `false` by [$\mathbf{0}$.]{.m}

Second, symbolically write that a proposal $\yY$ is `true`, given a conditional $\yX$, as follows:
$$
\tru(\yY \| \yX) = 1
$$
or ["$=0$"]{.m} if it's `false`.

The rules of truth-inference are then encoded by the following equations, which must always hold for any atomic or complex sentences [$\yX,\yY,\yZ$:]{.m}
...
:::
Therefore
[$\color[RGB]{238,102,119}\tru(\lnot\yh \| \ys \land \yf \land \yI) = 1$.]{.m}
...
:::{.callout-caution}
## {{< fa user-edit >}} Exercise
Prove the target inference $\color[RGB]{238,102,119}\tru(\lnot\yh \| \ys \land \yf \land \yI) = 1$ using the rules of truth-inference, but beginning from the starting inference [$\color[RGB]{34,136,51}\tru(\lnot\yh \land \lnot\ys \| \yf \land \yI)=1$.]{.m}
:::
