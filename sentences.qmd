...
Some expressions, even involving technical terms, may in fact appear to be sentences at first; but a deeper analysis may reveal that they are not. A famous example is the sentence "[The two events (at different spatial locations) are simultaneous]{.midgrey}". Einstein showed that there's no physical way to determine whether such an expression is true or false. Its truth turns out to be a matter of convention (also in Newtonian mechanics). The Theory of Relativity was born from this observation.
...
In some problems it's useful to introduce symbols to denote sentences. In these notes we'll use sans-serif italic letters: $\se{A},\se{B},\se{a},\se{b},\dotsc$,, possibly with sub- or super-scripts. For instance, the sentence "[The speed is 10 m/s]{.midgrey}" could be denoted by the symbol $\se{S}_{10}$. We abbreviate such a definition like this:

$$
\se{S}_{10} \defd \pr{The speed is 10\,m/s}
$$

which means "the symbol $\se{S}_{10}$ is defined to be the sentence $\pr{The speed is 10\,m/s}$”.
