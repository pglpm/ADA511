...
Second, it is extremely important in AI and machine-learning design. A (human) engineer who draws inferences, may proceed informally, without worrying about "sentences" unless some peculiar situation and a need for disambiguation arise. A data engineer who's *designing* or *programming* an algorithm that will do inferences automatically, must instead be unambiguous and cover beforehand all possible cases that the algorithm will face.
...
We must be careful about how much we shorten sentences. Consider these three:
$$
\begin{aligned}
...
\end{aligned}
$$
the quantity is the same in all three sentences, but their meanings are very different. The differences in meaning greatly affect any inference about or from thes sentences. For instance, in the third case an engineer may not take the indirectly-reported speed "10 m/s" at face value. In a scenario where all three sentences are relevant, it would be ambiguous to simply write ["$v = 10\,\mathrm{m/s}$"]{.together}: does the equal-sign mean "measured", "set", or "indirectly reported"?
...
We'll now see how more complex sentences are built from simpler ones. No matter whether complex or simple, any sentence can be represented by symbols like the ones above.
