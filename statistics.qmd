...
The population data is stored in the file [`hotel_bookings-market.csv`](datasets/hotel_bookings-market.csv). Each row of the file corresponds to a unit, and lists the unit id (this is not a variate in the present population) and the market segment.
...
2.
    ...
    - [*The frequency of a value in a population is an objective physical quantity.*]{.purple} All agents agree on the frequency (if they have the possibility to counting the units it).
...
- Download again the dataset [`income_nominal_data_all.csv`](datasets/income_nominal_data_all.csv):
...
Sometimes one can only share a more limited amount of information about the frequency distribution, or wants to emphasize particular aspects of it -- which are also aspects of the population. Different kinds of aspects can be chosen; some of them are only defined for specific types of variates. They are often called "summary statistics" or "descriptive statistics". Below we give a brief description of some common ones, emphasizing when they are appropriate and when they are not. These summaries can also be used for probability distributions.
...
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious
[*Some paradoxes, errors, and resolutions concerning the spectral optimization of human vision*](https://hvl.instructure.com/courses/25074/modules/items/713058)
:::
::::
...
- § 2.6 of [*Risk Assessment and Decision Analysis with Bayesian Networks*](https://hvl.instructure.com/courses/25074/modules/items/664427)
