...
In an [**ensemble plot**]{.blue}, the probability density is (approximately) represented by a collection of probable lines, or points, or similar objects. Plots like the ones above are examples (only one, though, correctly represent the density represented by the curve).

As the experiment and exercise above may have demonstrated, line plots sometimes give us slightly misleading ideas of how the probability is distributed across the domain; for example, peaks at some values make us overestimate the probability density around those values. Ensemble plots may give a more tangible representation of the probability distribution.
