...
and an analogous calculation yields $\P(F\mo\vfb \| \yR_1\mo\yy \and \yul)=33.333\%$.
...
Think about it: How can we say that an algorithm is over- or under-fitting, given that we do not know the ground truth? (if we knew the latter we would not be making inferences.) Such judgement reveals that we have some *preconceived* notion of what a reasonable distributions would look like.
...
::: {.callout-tip}
## {{< fa rocket >}} For the extra curious
[*Bayesian statistical inference for psychological research*](https://hvl.instructure.com/courses/25074/modules/items/721221)
:::
::::
