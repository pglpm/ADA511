...
Take a sequence of observations for $N$ units, for which the variate $\bZ$ is seen to have values $\blue z_1, z_2, \dotsc, z_N$. Some or even all of these $N$ values might be identical; denote by $\#\bz$ the multiplicity with which value $\bz$ occurs in the sequence. For instance, in the cube-collection example, the sequence

$$
C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yn \and C_4\mo\yy
$$
...
$$
C_{32}\mo\yn \and C_{102}\mo\yn \and C_{8}\mo\yn
$$
...
\begin{aligned}
\P(
\underbracket[0.1ex]{C_1\mo\yy \and C_2\mo\yy \and C_3\mo\yn \and C_4\mo\yy}_{
...
\sum_{\alpha=-1}^{2}
\frac{
\bigl(2^{\alpha} + 3 - 1\bigr)! \cdot
\bigl(2^{\alpha} + 1 - 1\bigr)!
}{
...
- $\P(C_1\mo\yy\| \yK)$ ; does the result make sense?

- $\P(C_{32}\mo\yn \and C_{102}\mo\yn \and C_{8}\mo\yn\| \yK)$
