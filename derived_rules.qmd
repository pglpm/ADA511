...
## Generalized `and`-rule {#sec-generalized-and}

The `and`-rule extends in an easy way to any number of and-ed sentences. For example, for four sentences

$$
\begin{aligned}
\P(\yX_1 \land \yX_2 \land \yX_3 \land \yX_4 \| \yZ) =
\P(\yX_1 \| \yX_2 \land \yX_3 \land \yX_4 \land \yZ) \cdot {}&
\\
\P(\yX_2 \| \yX_3 \land \yX_4 \land \yZ) \cdot {} &
\\
\P(\yX_3 \| \yX_4 \land \yZ) \cdot {} &
\\
\P(\yX_4 \| \yZ) &
\end{aligned}
$$
...
$$
\P(\yX_1 \land \yX_2 \land \yX_3 \| \yZ) =
\P(\yX_1 \| \yX_2 \land \yX_3 \land \yZ) \cdot
\P(\yX_2 \| \yX_3 \land \yZ) \cdot
\P(\yX_3 \| \yZ)
$$

(Hint: start by treating $\yX_2 \land \yX_3$ as one sentence and apply the usual `and`-rule.)

:::


## Generalized `or`-rule {#sec-generalized-or}
...
$$
\begin{aligned}
&\P(\yX_1 \lor \yX_2 \lor \yX_3 \lor \yX_4 \| \yZ) = {}
\\[1ex]
&\quad
\quad\P(\yX_1 \| \yZ) +
\P(\yX_2 \| \yZ) +
...
&\quad
{}+ \P(\yX_1 \land \yX_2 \land \yX_3 \| \yZ)
{}+ \P(\yX_1 \land \yX_2 \land \yX_4 \| \yZ)
{}+ \P(\yX_1 \land \yX_3 \land \yX_4 \| \yZ)
{}+ \P(\yX_2 \land \yX_3 \land \yX_4 \| \yZ)
\\[1ex]
&\quad
{}-\P(\yX_1 \land \yX_2 \land \yX_3 \land \yX_4 \| \yZ)
\end{aligned}
...
(Hint: start by treating $\yX_2 \lor \yX_3$ as one sentence and apply the usual `or`-rule. In the expression that you obtain, try to use the `or`-rule again; then the law from Boolean algebra

$$
\yX_1 \land (\yX_2 \lor \yX_3) =
(\yX_1 \land \yX_2) \lor (\yX_1 \land \yX_3) \,,
$$
...
- Confirm the result above using the [`inferP()`](https://github.com/pglpm/ADA511/blob/master/other_code/inferP.R) function introduced in [§@sec-algorithm-rules].
:::
