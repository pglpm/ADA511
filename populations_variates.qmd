...
- *variate:* the joint variate $(\vRI, \vCa, \vSi, \vType)$ consisting of four simple variates:
	...
	+ weight percent of [$\vSi$licon](https://pubchem.ncbi.nlm.nih.gov/element/14) in the fragment (interval discrete variate), with domain from $0$ to $100$ in steps of 0.01
...
A conditional frequencies can be calculated as the sum of the joint frequencies for all values ${\green x}$, in a way analogous to marginal probabilities ([§ @sec-marginal-probs]):

$$
f({\red Y\mo y}) = \sum_{\red y} f({\red Y\mo y}, {\green X\mo x})
$$

For example, if from the population of [table @tbl-glass] we consider the marginal population with variates $(\vCa, \vType)$ marginal population, we find the marginal frequencies
