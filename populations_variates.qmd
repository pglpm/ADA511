...
: We are interested in the daily change in [closing price](https://www.investor.gov/introduction-investing/investing-basics/glossary/closing-price) of a stock, during 100 days. Each day the change can be [positive (or zero)]{.green}, or [negative]{.yellow}.
    ...
    The daily change on any day can clearly be considered as a binary quantity, say with domain $\set{{\green\cat{+}}, {\yellow\cat{-}}}$. The daily changes in 100 days are a set of 100 binary quantities with exactly the same domain -- even if each one can have a different value.
