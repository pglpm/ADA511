...
Now we know how to represent arbitrarily complex sentences and express them in symbols. Let's introduce a way to clearly distinguish those that constitute the *assumptions* from those that constitute the *conclusion* in a specific decision problem. In the CPU example above, suppose that the statement we wrote down symbolically constitutes the assumptions. The conclusion to be inferred is $\se{c}_{2,90}$:\ \ $\pr{CPU\,2 is consuming 90\,W}$.

To distinguish assumptions and conclusions we can simply use the symbol\ \ "$\color[RGB]{238,102,119}\pmb{\|[\big]}$",\ \ a vertical bar^[Notation in formal logic uses the symbols\ \ $\models$\ \ or\ \ $\vdash$,\ \ and writes assumptions on the *left*, conclusions on the *right*. We use the notation used in probability logic.]:

- on its *left* side we write the sentence representing the *conclusion*,
...
In a decision problem we have a set of different conclusions that we want to assess given the same assumptions:
$$\begin{aligned}
\textit{\small conclusion\,1} &\pmb{\|[\big]} \textit{\small assumptions}
\\
\textit{\small conclusion\,2} &\pmb{\|[\big]} \textit{\small assumptions}
\\
\textit{\small conclusion\,3} &\pmb{\|[\big]} \textit{\small assumptions}
\\
\dotso&
\end{aligned}$$

The first goal in a decision problem is to assess these conclusions. What do we mean by "assess"? We cannot demand that the truth or falsity of the conclusions be determined with certainty. 
