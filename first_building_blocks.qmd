...
We shall first discuss how to represent and work with the *agent*, *assumptions*, *unknowns*, *probability* elements, leaving the *actions* and *utilities* to later.
...
## Assumptions, unknowns: sentences

The "assumptions" often include all data information we're sure about, or part of them. There is a difference between "assumptions" and "knowns", however.

In approaching a decision problem, and especially when considering decision sub-problems of a larger problem, the agent must often reason *hypothetically* or *counterfactually*.

For instance, consider an aeronautics problem where it must be decided whether to replace the fuel currently employed in a particular aircraft model, with a newly produced fuel type. To assess the consequence of employing the new fuel, the engineer must momentarily imagine that the new fuel is actually used and assess thermodynamic, environmental, and economic consequences. This is an example of *hypothetical reasoning*. Sometimes this hypothetical reasoning is assisted by performing experiments in restricted conditions, in which the new fuel is really used. Such supporting experiments, however, may not be viable, and in any case they are not full reflections of the hypothetical situation. The engineer may also have data from another aircraft model for which the new fuel was ultimately *not* used, and may try to assess what the consequences of the new fuel would have been *if it had been replaced* in that model, because such assessment may be easier post-facto. This is an example of *counterfactual reasoning*.

In either instance, the engineer sets up a decision problem in which the assumptions include actual data but also a situation that in one case is only imagined, and in the other case never happened. Both kinds of reasoning are staples of scientific research.




*Data* in particular are included among the assumptions, but also meta-data, and all that kind of information that cannot be exactly quantified.

The "unknowns" consist of information that we lack but would like to have, such as hypotheses and conjectures that we need to assess, verify or disprove. In most engineering problems these hypotheses ultimately concern the consequences of the available actions or unknown aspects that affect those consequences.

The distinction between "assumptions" and "unknowns" 
