...
Now we know how to represent arbitrarily complex sentences and express them in symbols. Let's introduce a way to clearly distinguish those that constitute the *assumptions* from those that constitute the *outcome* in a specific decision problem. In our assembly-line example, suppose that the statement which we have just written down in symbol constitutes the assumptions. The outcome to be inferred is expressed by the sentence
$$
...
The collection of assumptions on the right side of the bar ["$\pmb{\|[\big]}$"]{.together} is called the [**conditional**]{style="color:#4477AA;"}. The expression above is read\
