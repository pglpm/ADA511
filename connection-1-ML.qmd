# [Connection with machine learning and AI]{.midgrey} {#sec-connection-1-ML}
{{< include macros.qmd >}}
...
:::{.callout-note}
##  
You find the code for this chapter and exercises also in [this JupyterLab notebook for R](other_code/mlc_vs_opm.ipynb) and (courtesy of Viktor Karl Gravdal!) [this JupyterLab notebook for python](other_code/mlc_vs_opm_py.ipynb).
:::
...
Their operation is implemented in [this R script](other_code/mlc_vs_opm.R) that we now load:
...
An example of such confusion is with present-day *large language models* (LLMs, which we also discuss in chapters [-@sec-example-slm] and [-@sec-example-slm2]) and in particular those with a Generative Pre-training Transformer (GPT) architecture. In many media they are referred to as "AI systems"; yet what they achieve is not *intelligence*, but rather *human-like* language processing -- including non-intelligent processing.

If you have access to a large language model, you have surely witnessed examples of nonsensical output^[often euphemistically called "hallucination" because this term may increase sales, whereas "stupid" would risk decreasing sales.]. You can try a variation of the following experiment:
...
- Turing 1950: [*Computing machinery and intelligence*](references.html); a beautifully written paper!
