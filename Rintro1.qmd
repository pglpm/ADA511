...
```
You notice the following features:

- Many functions we run, such as `seq()`, have *named arguments*, for example `from = -5`, or `to = 6`, or `xlab = 'x'`. In some cases the argument names can be omitted; for instance we could write `seq(-5, 6, by = 0.5)`. But in these cases one must pay attention to the order of the arguments.
...
- Familiarize yourself with basic mathematical operations in R, like `+`, `-`, `*`, `/`, `^`, `exp()`, `log()`.

- Familiarize yourself to assigning single values and sequences of values to variables.
...
- What happens if you omit the vertical-axis range `ylim = c(-6, 35)`?
