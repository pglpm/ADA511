...
- [Pinference](https://cran.r-project.org/package=Pinference)
...
We shall also use some custom-made functions for plotting and reading data. They are defined in the [`pplot.R`](code/pplot.R) file, which you should download to your working directory. Once it's downloaded, you can load the functions this way:
...
- In some circumstances it is possible to break a command over several lines; for instance, the arguments to the `pplot()` functions were not all given in one line. One must be careful because linebreaks do terminate some expressions; but in general it is safe -- and clearer for people who read your code -- to distribute function arguments across several lines, as done above.
