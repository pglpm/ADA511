...
In an [**ensemble plot**]{.blue}, the probability density is (approximately) represented by a collection of probable lines, or points, or similar objects, as in the examples above (only one of the examples above, though, correctly matches the density represented by the curve).
...
Ensemble plots are also useful for representing probability densities in more than one dimension -- sometimes even in infinite dimensions! They can moreover be easier to produce computationally than line plots.
