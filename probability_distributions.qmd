...
In the [International System of Units (SI)](https://www.nist.gov/pml/owm/metric-si/si-units)^[see also the material at the [International Bureau of Weights and Measures (BIPM)](https://www.bipm.org/en/measurement-units)], "Degree of belief" is considered to be a dimensionless quantity, or more precisely a quantity of dimension "1". This is why we don't write units such as "metres" ($\mathrm{m}$), "kilograms" ($\mathrm{kg}$) or similar together with a probability value.
...
It is an error to report probability densities without their correct physical units; in fact, keeping track of these units is often useful for doing consistency checks and finding errors in calculations, just like in other engineering or physics calculations.

On the other hand, if we write probability densities as suggested in the previous section, that is in this case as "$\p(T \mo 20\mathrm{°C} \| \yI)\,\di T$", then we do *not* need to report physical units, because the density written this way is a pure probability. We can therefore write

$$
\p(T \mo 20\mathrm{°C} \| \yI)\, \di T = 1.2
$$

where the units "$\mathrm{°C^{-1}}$" have disappeared because multiplied by $\di T$, which has the inverse units $\mathrm{°C}$.
