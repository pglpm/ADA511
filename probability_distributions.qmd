...
For a temperature measurement, for instance, the cases could be "[The temperature is measured to have value 271 K]{.green}", "[The temperature is measured to have value 272 K]{.green}", and so on up to 275 K. These cases are expressed by mutually exclusive and exhaustive sentences. Denoting th temperature with $T$, these sentences can be abbreviated as
$$
...
The agent's belief about the quantity is then expressed by the [probabilities]{.purple} about these sentences, conditional on the agent's state of knowledge [${\color[RGB]{204,187,68}\yI}$:]{.m}
<!-- 4 10 18 28 40 -->
$$\begin{aligned}
\P({\color[RGB]{34,136,51}T \mo 271\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) &= {\color[RGB]{170,51,119}0.04} \\[1ex]
\P({\color[RGB]{34,136,51}T \mo 272\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) &= {\color[RGB]{170,51,119}0.10} \\[1ex]
\P({\color[RGB]{34,136,51}T \mo 273\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) &= {\color[RGB]{170,51,119}0.18} \\[1ex]
\P({\color[RGB]{34,136,51}T \mo 274\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) &= {\color[RGB]{170,51,119}0.28} \\[1ex]
\P({\color[RGB]{34,136,51}T \mo 275\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) &= {\color[RGB]{170,51,119}0.40}
\end{aligned}$$
that sum up to one:
$$\begin{aligned}
&\P({\color[RGB]{34,136,51}T \mo 271\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) +
\dotsb +
\P({\color[RGB]{34,136,51}T \mo 275\,\mathrm{K}} \| {\color[RGB]{204,187,68}\yI}) \\
&\quad{}= 
...
In the case of ordinal or interval quantities it is sometimes possible to express the probability as a *function* of the value. For instance, the probability distribution above could be summarized by this function of the value [${\color[RGB]{34,136,51}t}$:]{.m}
$$
\P({\color[RGB]{34,136,51}T\mo t} \| {\color[RGB]{204,187,68}\yI}) = 
...
$$\p(X\mo x \| \yI)\,\di x$$
where ["$\di x$"]{.m} stands for the width of a small range around $x$. This notation is also helpful with integrals. Unfortunately it becomes a little cumbersome when we are dealing with more than one quantity.
...
We want to represent the amount of probability in any small range, say between $X\mo 0$ and [$X\mo 0.1,$]{.m} by drawing in that range a number of short thin lines, the number being proportional to the probability. So a range with 10 lines has twice the probability of a range with 5 lines. The probability density around a value is therefore roughly represented by the density of the lines around that value.
...
\end{aligned}$$
which we can shorten to\ \ [$F\mo 1,\ F\mo 2,\ \dotsc,\ F\mo 24$.]{.m} Note the slightly different meaning of the last value.
...
Suppose that the inspection device -- our agent -- has internally calculated a probability distribution for $F$, conditional on its internal programming and the results of the tests on the component, collectively denoted [$\yI$.]{.m} The probabilities, compactly written, are
$$
\P(F\mo1 \|\yI), \quad
\P(F\mo2 \|\yI), \quad
\dotsc, \quad
...
What's important for the agent's decision about rejecting or accepting the component, is not the exact time when it will fail, but only whether it will fail within the first year or not. That is, the agent needs the probability of the sentence $\pr{The component will fail within a year of use}$. But this sentence is just the `or` of the first 12 sentences expressing the values of $F$:
$$
\begin{aligned}
...
\\[1ex]&\qquad{}\equiv
(F\mo 1) \lor (F\mo 2) \lor \dotsb \lor (F\mo 12)
\end{aligned}
$$
The probability needed by the agent is therefore
$$
\P(F\mo1 \lor F\mo2 \lor \dotsb \lor F\mo12\| \yI )
$$
...
\P(F\mo1 \lor F\mo2 \lor \dotsb \lor F\mo12\| \yI )
\\[1ex]&\qquad{}=
\P(F\mo1 \|\yI) + \P(F\mo2 \|\yI) + \dotsb + \P(F\mo12 \|\yI)
\\[1ex]&\qquad{}= \sum_{f=1}^{12} \P(F\mo f \| \yI)
\end{aligned}
$$
