...
## Representation of probability distributions: importance for our AI-agent project {#sec-discr-prob-distr-important}

We shall now explore different ways to represent and visualize distributions of probability. Although this topic may only appear as a collection of facts, definitions, examples, and curiosities, it will actually be extremely important when we build and employ our prototype AI agent in later chapters, especially around [Ch. @sec-example-opm1]. What you learn in the present chapter *will allow you to visualize the AI-agent's uncertainty* in its logical reasoning and decisions. Our AI agent will have to reason and make guesses about complicated quantities, like the future frequencies of particular kinds of events, or the frequencies of different kinds of individuals in a large population. Its uncertainty about such quantities can of course be read and shown numerically, but managing to *visualize it* will help us understand more intuitively its state of knowledge and of uncertainty, and the logic behind its actions. So don't undervalue what you learn in the present chapter!

Let's see how probability distributions can be represented and visualized for the basic types of quantities discussed in [§@sec-quantities-types-basic], and how these representations can be generalized to more complicated quantities.

We start with probability distributions over discrete domains.

## Representation of discrete probability distributions {#sec-discr-prob-distr}
...
A probability distribution for a nominal, ordinal, or discrete-interval quantity can be neatly represented by a [**histogram**]{.blue} (alternatively called *bar chart* in some texts, sometimes with fine distinctions between the two terms):
...
In these cases we can use standard [**line plots**]{.blue}, or variations thereof. Compare the following example.
...
The histogram and the line-plot representations become indistinguishable for a probability density.
...
### Ensemble plots {#sec-scatter-plot}

Curve-based plots of a probability density are very informative, but they can also be slightly deceiving. Try the following experiment.

Consider a continuous quantity $X$ with the following probability density:

![](example_curve_density.png){width=100%}

We want to represent the amount of probability in any small range, say between $X\mo 0$ and $X\mo 0.1$, by placing a number of short thin vertical lines in that range, *the number being proportional to the probability*. So a range containing 10 lines has twice the probability of a range containing 5 lines. The probability density around a value is therefore roughly represented by the density of the lines within a small range around that value.

Suppose that we have 50 lines available in total to distribute this way. How should we place them around? Check these four attempts:
...
:::::{style="font-size:110%;"}
Which of these plots has the most correct placement of the 50 lines? [(NB: the position of the correct answer is determined by a pseudorandom-number generator.)]{.small}
...
In an [**ensemble plot**]{.blue}, the probability density is (approximately) represented by a collection of values "sampled" from the distribution, in the sense that values with higher probability are proportionally more likely to be sampled. These values can be graphically represented by lines, or points, or more complex objects.

As the experiment and exercise above may have demonstrated, curve-based plots sometimes give us slightly misleading ideas of how the probability is distributed across the domain. For example, peaks at some values make us overestimate the probability density around those values. Ensemble plots often give a less misleading representation of the probability density.

Ensemble plots are also useful for representing probability densities over domains that have more than one dimension -- sometimes even in infinite dimensions! They can moreover be easier to produce computationally than curve-based plots. In later chapters we'll discuss probability distributions *over frequency distributions*, and essentially the only way to represent such a probability distribution is by an ensemble plot; in this case the samples in the ensemble are themselves distributions. This is why it's important that you get a clear understanding of the idea behind ensemble plots.
...
[**Read**]{.lightblue} §§ 5.3.0--5.3.1 of Fenton & Neil: [*Risk Assessment and Decision Analysis with Bayesian Networks*](references.html)
:::
