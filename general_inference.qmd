...
where $\P(F\mo\vf\|\yI)$ is problem-dependent and must be specified by the agent. In the case of a single or joint nominal variate $\bZ$ we also have an explicit mathematical formula for an agent that adopts a Dirichlet-mixture belief distribution ([§ @sec-prob-for-freqs]).


Th formula above can be used to draw two main kinds of inferences:
