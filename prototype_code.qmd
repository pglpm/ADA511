...
A concise documentation is here given of the R functions designed in [ch. @sec-code-design] and described in [§@sec-code-writing], together with an example of how they are used in a task.

The functions are collected in
[`https://github.com/pglpm/ADA511/blob/master/code/OPM_nominal.R`](https://github.com/pglpm/ADA511/blok/master/code/OPM_nominal.R)
...
{{< fa code >}} `buildagent(metadata, data = NULL, kmi = 0, kma = 20, savememory = FALSE)`
: Arguments:
	...
	- *`kmi`*: the $\amin$ parameter.
	- *`kma`*: the $\amax$ parameter.
...
{{< fa code >}} `infer(agent, predictand, predictor = NULL)`
: Arguments:
...
{{< fa code >}} `rF(n = 1, agent, predictand, predictor = NULL)`
...
: Background and metadata information, as well as auxiliary assumptions, are collected, examined, prepared. Remember that this kind of information is required in order to make sense of the data ([§@sec-underlying-distribution]). In this stage we ask questions such as ["Is our belief about the task exchangeable?"]{.midgrey}, ["Can the statistical population be considered infinite?"]{.midgrey}, and similar question that make clear which kinds of ready-made methods and approximations are acceptable or not. This stage also helps for correcting possible deficiencies in the training data used in the next stage. For instance, some possible variate values might not appear in the training data, owing to their rarity in the statistical population.
	...
	- possible actions that may be required in the repeated task applications
	- utilities associated with the actions above
...
If no or few training data are available, then the optimal agent will automatically absorb as much information as possible from them, and combine it with its background information to draw optimal inferences and take optimal actions.
...
: Inferences are drawn, and action are taken, for each new application instance. With our prototype agent, the inferences and the actions can in principle be different from instance to instance.
