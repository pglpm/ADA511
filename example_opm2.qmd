# [The prototype agent makes decisions]{.red} {#sec-example-opm2}
{{< include macros.qmd >}}
...
We use the targeted-advertisement scenario of [§ @sec-max-exp-util-example].

Utility matrix: assumed to be the same at each instance
...
Same example as in [§ @sec-max-exp-util-example]:
...
Find expected utilities by matrix multiplication
...
The function that executes maximization of expected utility is `decide()`:
...
We build the analogous of a "confusion matrix", telling us how many times the agent chooses the three ad-types for both income levels. (Use a for-loop for clarity)
...
for(i in 1:M){
    ...
    probs <- infer(agent=opmall, predictand='income',
                   predictor=userpredictors)
    decision <- decide(utils=adutilities, probs=probs)
...
Note that the yield is higher than what would be obtained by just choosing the neutral ad-type `B` for all test units (the average yield would be exactly `2`).
