# [The prototype Optimal Predictor Machine takes action]{.red} {#sec-example-opm2}
{{< include macros.qmd >}}
...
- `EUs`: the expected utilities of the actions, sorted from highest to lowest
- `optimal`: one action unsystematically chosen among the optimal ones (if more than one)
...
for(i in 1:ntest){
    ...
    action <- decide(utils = adUtilities, probs = probs)$optimal

    trueincome <- testdata[i, 'income']

    confusionmatrix[action, trueincome] <-
        confusionmatrix[action, trueincome] + 1
}
