...
So far we have been jumping back and forth between "inference" and "data", keeping the two topics somewhat separated. Now we shall finally use them together in more realistic scenarios. But before continuing let us summarize what we have learned so far; also to motivate why those two topics have been kept separated.

On the "inference side" we first gave a concrete and operational meaning to "drawing an inference", as the calculation of the probability -- the degree of belief -- of some sentence from the probability of others. This means that in principle we can draw inferences and apply probability theory to *literally anything* that can be expressed by language.
...
- No inference can be drawn unless some probabilities are first posited as a starting point. This is just another face of the formal-logic fact that no theorem (besides tautologies) can be derived, unless some axioms are given first.
...
On the "data side" we introduced a set of notions, such as quantity, having a value, domain, quantity type, population, variate, frequency. We learned how to use them and paid attention to some of their counter-intuitive properties.

These notions allow us to *speak*, in a somewhat precise way, about typical situations and problems that arise in engineering and other scientific contexts. In essence all we did was to introduce a particular kind of *sentences* to express engineering problems -- *so we can apply the probability calculus to them* and draw inferences about them.

```{mermaid}
flowchart LR
  A[data notions] --> B(sentences)
  B --> C[probability calculus]
```
