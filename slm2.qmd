...
## Agents that generate text: what's the underlying decision-making problem? {#sec-what-decision-llm}

In [§@sec-language-inference] we saw that present-day Large Language Models determine their belief in what the next word or token should be, based only on the sequence of words seen so far, and on the frequencies of such sequences in a huge collection of texts. This belief is not determined by possible *future* outcomes of the choice of words, as instead is the case in human conversation. We managed to use our Optimal Predictor Machine in a way somewhat similar to a Large Language Model, in a simplified setting.

Large Language Models do not output probabilities, however: they output words. So at every step they are effectively *choosing* one of the possible words about which they calculated their beliefs. This means that there's an underlying choice; and to be optimal and self-consistent, such choice should be based the the utilities of some outcomes.
...
Let's consider our OPM agent used as a "small language model" in [§@sec-opm-as-slm]. It was only calculating degrees of belief about the $n$th token of a string of $n$ tokens. First of all let's ask again: beliefs about what? We could say: the belief that this $n$th token is the correct one, in this particular sequence. Keep in mind that this is a suspicious point of view, though: can we really say that there's a "correct" token?
...
These conditions suggest that we take an "*unpredictable, unsystematic action*", in the sense discussed in [§@sec-unpred-decisions].

Let's introduce the following action:

["Unpredictably select and output the next token according to its belief"]{.yellow}.

In the long run, taking this action will produce text that is unpredictable, and also whose 3-gram frequencies reflect those of the training texts, because the agent bases its beliefs on those (though beliefs and frequencies are not exactly equal). Therefore it seems that, in the long run, this action will lead to the highest utility.
\

The function `generatetext()` in the file [`SLMutilities.R`](https://github.com/pglpm/ADA511/blob/master/code/SLMutilities.R) applies the heuristic strategy above and outputs the resulting text. We saw an example output in [§@sec-preview-gentext]. Let's see another example output:
...
:::{.callout-important}
## {{< fa exclamation-triangle >}} We're approximating

The discussion above is **not a rigorous application of the principle of maximal expected utility**. We did not list properly the actions, or the outcomes, or the utility values. The reasoning was purely heuristic and intuitive.
...
- Reverse-engineer the function `generatetext()` and verify that it indeed implements the action discussed above.
