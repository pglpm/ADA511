...
:::{.callout-caution}
## {{< fa user-edit >}} Exercise
- Do the "discriminative" inferences above, but omitting from the `newfragment` data *one* variate in turn; so first omit only $\vRI$ and do the inferences, then omit only $\vNa$ and do the inferences, and so on.
