...
The plot above and the information is represents are a very useful for inference purposes: not only it gives the probability for the next observation, but it also gives an idea of how such a probability could change in the future, as more data are collected and knowledge of the population's frequency distribution becomes more precise.
...
## The agent learns from the sample data
...
## Conditional or "supervised" inferences

The inferences about a new units that the agent has made so far were of an "unsupervised" or "generative" kind ([§ sec-2nd-connection-ML]): the agent did not receive or use any partial information about a new unit.

Let's now imagine that we are at a new crime scene, a glass fragment is recovered, and tests are made about its refractive index and chemical composition. The following values are found, referred to the levels of our variates:
...
The `condknowledge` object contains the agent's knowledge conditional on the variates given; this knowledge is about the remaining variates, which in this case are the single variate $\vType$ (so we don't need to calculate any marginal distributions).
...
```{r}
plotsamples1D(finfo=condknowledge)
```
...
Should the crime investigation proceed on the assumption that the fragment is of $\cat{T2}$ type? No, not necessarily. The best decision depends on the gains and costs involves in making correct or wrong assumptions. To this last decision-making problem we turn next.
