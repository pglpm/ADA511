...
[First let's load some libraries and functions that we shall use in this example: the [`pplot.R`](code/pplot.R) and the [`OPM_nominal.R`](code/OPM_nominal.R) scripts, which should be `source`d from an R console:]{.small}
...
```
source('pplot.R')
source('optimal_predictor_machine_nominal.R')
```
...
We would like to visualize this probability distribution over marginal frequency distributions. A complication is that we would need infinite dimensions to visualize this faithfully. One approximate way to represent this probability distribution is by showing, say, 100 representative samples from it. The idea is the same as for an ensemble plot ([§@sec-represent-dens]). In this case we would then have 100 different frequency distributions for the variate $\vType$.
...
The inferences about a new units that the agent has made so far were of an "unsupervised-learning" or "generative" kind ([§sec-connection-3-ML]): the agent did not receive or use any partial information about a new unit. Let's now try a "supervised-learning" or "discriminative" kind of inference.
