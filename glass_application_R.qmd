...
[First let's load some libraries and functions that we shall use in this example: the [`tplotfunctions.R`](code/tplotfunctions.R) and the [`OPM_nominal.R`](code/OPM_nominal.R) scripts, which should be `source`d from an R console:]{.small}
