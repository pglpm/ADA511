# [Limitations of present-day machine-learning algorithms]{.darkgrey} {#sec-ML-utility-limitation}
{{< include macros.qmd >}}
...
- [{{< fa arrows-split-up-and-left >}}\ \ a list of possible actions]{.yellow}

- [{{< fa sack-dollar >}}\ \ the utilities of the actions, depending on the predictand's true value]{.blue}
...
Which of the treatments above should the clinician choose? We now know how the clinician should make the optimal action, but let's explore the possible consequences of not making it, in three different scenarios. In each scenario, the clinician has prescribed several clinical tests (the predictors) for the patient, and obtained their results.
...
Clearly the best action is treatment $\yA$. It is possible that, unfortunately, the present patient's life will be shortened; but this treatment was the patient's and clinician's best bet.
...
The best action is treatment $\yB$.
...
Treatment $\yC$ is the best action in this scenario, for reasons complementary to those of scenario 1.
...
In each scenario, note that *any* action strategy different from "sticking to the optimal action" leads to suboptimal results -- that is, lives shortened more than what was unavoidable. In scenario 1, for instance, an action strategy such as "[choose treatment $\yA$ most of the time, and treatment $\yB$ from time to time]{.midgrey}" leads to an additional life shortening of several months. This is clear from the following graph, which shows the average reduction in life expectancy for each treatment, depending on the percentage of patients with the disease:
...
The clinical example above shows that there isn't any one-to-one connection between actions and unknowns. We cannot say, for instance, that treatment $\yC$ "corresponds" to the presence of the disease, because the best treatment is actually $\yB$, not $\yC$, if the probability for $\dise\mo\yy$ is above 50% but below 75%.

<!-- Many or most present-day machine-learning algorithms focus on the unknown variate-value and don't yield any probabilities. This limitation makes it impossible to determine the optimal action; such algorithms sub-optimal by construction. -->
...
### [{{< fa exclamation-triangle >}}\ \ Output scores or weights are *not* probabilities!]{.red}
