...
It's very important to keep in mind that despite the use of the words "generate" and "guess", *both tasks above require the computation of probabilities*; that is, the agent must assess its beliefs. They are both *inference* tasks. A guessing task requires the agent to make an inference, that is, calculating its beliefs about various possibilities; whereas a generation task requires the agent to first make an inference, as in guessing, and then to *choose* only one of those possibilities. So roughly speaking we can say that "generation" is "guessing followed by an action".

For this reason we shall now focus on "guessing" tasks only.
...
Guessing or inference tasks can be further divided into two subtypes:
