...
## A task-oriented categorization of some inference problems {#sec-cat-problems}

There are all sorts of inferences in everyday life and in highly technological applications, in situations without serious consequences and in others, like medicine, where lives may be at stake.

Owing to such variety we cannot define clear-cut categories of inferences. But an informal categorization can provide a starting point to examine some new kind of inference task that we may have to face. Many tasks will fall in between categories; every data-engineering or data-science problem is unique.
...
:::{.column-margin}
Remember that you can adopt any terminology you like. If you prefer "instance" and "characteristics" or some other words to "unit" and "variate", then use them. What's important is that you understand the ideas and methods behind these words, and that you can explain what you mean to others.
:::
...
- [{{< fa sign-out-alt >}} {{< fa cube >}}\ \ Tasks where an agent must itself *generate* a new unit.]{.darkgrey}

- [{{< fa cube >}} {{< fa question >}}\ \ Tasks where a new unit is given to an agent, who must *guess* some of its variates.]{.darkgrey}

An example of the first type of task is image generation: an algorithm is given a collection of images and is asked to generate a new image based on, or "inspired" by, them.

It's very important to keep in mind that despite the use of the words "generate" and "guess", **both tasks above require the computation of probabilities**. So both are *inference* tasks. This is because both in generation and in guessing an agent needs to assess its degrees of belief -- about what's best to generate, or about the possible guesses.
...
:::{.small .midgrey}
In machine learning, the terms "discriminative" and "generative" are sometimes associated with the two types of task above. This association, however, is quite loose, because some tasks typically called "generative" actually belong to the first type. We therefore try to avoid this terminology here. It's enough to keep in mind the distinction between the two types of task above.
:::
...
- [{{< fa regular star >}}\ \ The agent must guess *all* variates of the new unit.]{.darkgrey}

- [{{< fa star-half-alt >}}\ \ The agent must guess *some* variates of the new unit, but can observe other variates of the new unit.]{.darkgrey}
...
- [{{< fa star-half-alt >}} {{< fa star-half-alt >}}\ \ All predictors and predictands of previous units are known to the agent]{.darkgrey}

- [{{< fa star-half >}} {{< fa star-half >}}\ \ All predictors of previous units, but not the predictands, are known to the agent]{.darkgrey}

- [{{< fa regular star-half >}} {{< fa regular star-half >}}\ \ All predictands of previous units, but not the predictors, are known to the agent]{.darkgrey}
...
### [{{< fa regular star >}}]{.darkgrey}\ \ The agent must guess *all* variates of the new unit
...
### [{{< fa star-half-alt >}}]{.darkgrey}\ \ The agent must guess *some* variates of the new unit, having observed other variates of the new unit
...
#### [{{< fa star-half-alt >}} {{< fa star-half-alt >}}]{.darkgrey}\ \ All predictors and predictands of previous units are known to the agent
...
#### [{{< fa star-half >}} {{< fa star-half >}}]{.darkgrey}\ \ All predictors of previous units, but not their predictands, are known to the agent
...
### [{{< fa star-of-life >}}]{.darkgrey} More general and hybrid tasks
...
### [{{< fa sign-out-alt >}}]{.darkgrey}\ \ The agent must *generate* a new unit
...
##  
::::{style="font-size:120%"}
Probability distribution such as those discussed above should intrinsically enter all types of inference algorithms, including machine-learning algorithms.
::::
:::

This is the condition for any inference algorithm to be optimal and self-consistent. The less an algorithm satisfies that condition, the less optimal and less consistent it is.
...
Using the marginalization rule we find these final expressions of the probabilities for some tasks discussed so far:

::::{.column-page-right}
:::{.callout-note}
##  

- [{{< fa regular star >}}]{.darkgrey} Guess all variates:
...
- [{{< fa star-half-alt >}} {{< fa star-half-alt >}}]{.darkgrey} All previous predictors and predictands known:
...
- [{{< fa star-half >}} {{< fa star-half >}}]{.darkgrey} Previous predictors known, previous predictands unknown:
