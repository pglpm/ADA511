...
There are all sorts of inferences that we need to draw, in everyday life and in highly technological applications, in situations without serious consequences and in others, like medicine, where lives may be at stake.

There are no distinct, clear-cut categories of inferences, but an informal categorization can provide a starting point from which to examine a new kind of inference task that we may face. Many tasks will fall in between categories: every data-engineering or data-science problem is unique.

The important questions for us are these:

- **What do we wish to infer or choose?**
- **From which kind of information?**

So let's focus on a categorization based on the purpose, and on the types of desired information and of available information. We *exclude* all tasks that require an agent to *continuously and actively interact with its environment* for acquiring information, making choices, getting feedback, and so on. These tasks are the domain of Decision Theory in its most complex form, with ramified decisions, *strategies*, and possibly the interaction with other decision-making agents. To explore and analyse this kind of tasks is beyond the purpose of this course.
